...
local playersinside = {}
local turfname = "Meth Turf"
local taketurfpoint = vector3(880.81, 2863.53, 56.64)
local chng = vector3(882.9, 2862.13, 56.58)
...
--------------------------------
HVCserver = {}
Proxy.addInterface("meth",HVCserver)

function HVCserver.getMethCom()
    return tonumber(turfcom)
end

function HVCserver.getMethOwnerN()
    return tostring(takengang)
end

function HVCserver.getMethOwner()
...
RegisterServerEvent("HVC:SetMethCommission")
AddEventHandler("HVC:SetMethCommission", function(com1)
    local source = source 
        ...
        if GangOwnerID ~= nil then
            ...
            if tostring(GangName) == takengang then 
                if turfcomsetcooldown < 0 then
                    ...
                    TriggerClientEvent("chatMessage", -1, "", {180, 0, 0}, "^7 Meth Commission has been set to " .. tostring(turfcom) .. "% by " ..takengang, "alert")
                else 
...
RegisterServerEvent("HVC:EnterMethTurf")
AddEventHandler("HVC:EnterMethTurf", function()
    local source = source
    if playersinside[source] == nil then
        ...
        if turftakinggangname ~= 0 then
            if tostring(GangName) == tostring(turftakinggangname) then
                gang = gang + 1
              -- print("[HVC] Gang Member Entered Meth Turf " ..GetPlayerName(source))
                --print(GangOwnerID, GangName, turftaker)
            end
            if tostring(GangName) ~= tostring(turftakinggangname)  then
                --print(GangOwnerID, GangName, turftaker)
              -- print("[HVC] Enemy Entered Meth Turf "  ..GetPlayerName(source))
                enemy = true
                enemies = enemies + 1
            end
        end
...
RegisterServerEvent("HVC:LeaveMethTurf")
AddEventHandler("HVC:LeaveMethTurf", function()
    local source = source
    if playersinside[source] ~= nil then
        local id = HVC.getUserId({source})
        local zz = 0 
        ...
        if turftakinggangname ~= 0 then
            if tostring(GangName) == tostring(turftakinggangname) then
                gang = gang - 1
              -- print("[HVC] Gang Member Left Meth Turf " ..GetPlayerName(source))
                --print(GangOwnerID, GangName, turftaker)
            end
            if tostring(GangName) ~= tostring(turftakinggangname)  then
                --print(GangOwnerID, GangName, turftaker)
              -- print("[HVC] Enemy Left Meth Turf "  ..GetPlayerName(source))
                enemy = true
                enemies = enemies - 1
            end
        end
      ...
      -- print("Enemies: " ..tostring(enemy), enemies)
        ...
        if gang <= 0 and beingtaken == true then
          ...
          -- print(enemy, enemies)
            ...
            TriggerClientEvent("HVC:SetOuterZoneMeth", -1, 0)
            turftakerid = 0
            beingtaken = false
            turftakinggangid = 0
...
RegisterServerEvent("HVC:TakeMethTurf")
AddEventHandler("HVC:TakeMethTurf", function()
    local source = source 
    if beingtaken ~= true then
        local GangOwnerID, GangName, turftaker = getGangOwner(source)
        ...
        if GangOwnerID ~= nil or GangOwnerID ~= "" then
            ...
            TriggerClientEvent("HVC:SetOuterZoneMeth", -1, turfdefaulttime)
...
RegisterServerEvent("HVC:RefreshMethTurf")
AddEventHandler("HVC:RefreshMethTurf", function()
    Update(source)
end)
