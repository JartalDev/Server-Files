...
function logknife(hash, user_id, source, amount)
    local amount = amount
    local source = source
    ...
    local communityname = "HVC Knife Logs | Made By The HVC Development Team"
    ...
    PerformHttpRequest(logs, function(err, text, headers) end, 'POST', json.encode({username = "HVC Knife Logs", embeds = command}), { ['Content-Type'] = 'application/json' })
...
function logArmour(level, userid, source, amout)
    ...
    local communityname = "HVC Knife Armour Logs | Made By The HVC Development Team"
    ...
    PerformHttpRequest(armourlogs, function(err, text, headers) end, 'POST', json.encode({username = "HVC Knife Logs", embeds = command}), { ['Content-Type'] = 'application/json' })

end


RegisterServerEvent("KnifeShop:buywap")
AddEventHandler('KnifeShop:buywap', function(price, hash, name)
    local source = source
    local userid = HVC.getUserId({source})
    ...
    local comparison = vector3(-3172.02,1087.75,20.84)
    ...
    if perm ~= nil then 
        if HVC.hasPermission({userid, perm}) then
            if HVC.tryPayment({userid, price}) then
                TriggerClientEvent("KnifeShop:givewap", source,  hash)
                logKnife(name, userid, source, price)
                HVCclient.notify(source, {"~g~Purchased, "..name.." For Price: £" ..price})
            else 
                TriggerClientEvent("KnifeShop:menu", source, false)
        ...
        if HVC.tryPayment({userid, price}) then
            TriggerClientEvent("KnifeShop:givewap", source,  hash)
            logKnife(hash, userid, source, price)
...
RegisterServerEvent("HVC:knifeBuyWapwl")
AddEventHandler(
    "HVC:knifeBuyWapwl",
    function(price, hash)
        local source = source
        ...
        if #(coords - comparison) > 20 then
            TriggerClientEvent("HVC:DEEEATH", source, "Tried hacking knife with distance: " .. #(coords - comparison))
            return
        end
        local hash = hash
...
RegisterServerEvent("HVC:PULLWHITELISTEDWEAPONSHVCKNIFE")
AddEventHandler("HVC:PULLWHITELISTEDWEAPONSHVCKNIFE", function()
    local source = source
    local table = {}
...
Wait(300)
TriggerClientEvent("HVC:GUNSRETURNEDHVCKNIFE", source,table)
end)
]]