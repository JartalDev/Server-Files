...
local perm = "large.license"

function loglarge(hash, user_id, source, amount)
    local amount = amount
    local source = source
    ...
    local communityname = "HVC Large Logs | Made By The HVC Development Team"
    ...
    PerformHttpRequest(logs, function(err, text, headers) end, 'POST', json.encode({username = "HVC Large Logs", embeds = command}), { ['Content-Type'] = 'application/json' })
...
function logArmour(level, userid, source, amout)
    ...
    local communityname = "HVC Large Armour Logs | Made By The HVC Development Team"
    ...
    PerformHttpRequest(armourlogs, function(err, text, headers) end, 'POST', json.encode({username = "HVC Large Logs", embeds = command}), { ['Content-Type'] = 'application/json' })

end


RegisterServerEvent("LargeArms:buywap")
AddEventHandler('LargeArms:buywap', function(price, hash, name)
    local source = source
    local userid = HVC.getUserId({source})
    ...
    if perm ~= nil then 
        if HVC.hasPermission({userid, perm}) then
            if HVC.tryPayment({userid, price}) then
                TriggerClientEvent("LargeArms:givewap", source,  hash)
                logLarge(name, userid, source, price)
                HVCclient.notify(source, {"~g~Purchased, "..name.." For Price: £" ..price})
            else 
                TriggerClientEvent("LargeArms:menu", source, false)
        ...
        if HVC.tryPayment({userid, price}) then
            TriggerClientEvent("LargeArms:givewap", source,  hash)
            logLarge(hash, userid, source, price)
    ...
    if perm ~= nil then 
        if HVC.hasPermission({playerid, perm}) then
            if HVC.tryPayment({playerid, price}) then
                TriggerClientEvent("LargeArms:givearmour", source, level)
                TriggerClientEvent("HVC:ServerGotArmd", source)
                logLarge(level, userid, source, price)
                HVCclient.notify(source, {"~g~Paid £"..price})
            else 
                TriggerClientEvent("LargeArms:menu", source, false)
                HVCclient.notify(source, {"~r~Insufficient funds"})
        ...
        if HVC.tryPayment({playerid, price}) then
            ...
            TriggerClientEvent("LargeArms:armour", source, false)
            HVCclient.notify(source, {"~r~Insufficient funds"})
        end
    end
end)
...
local weaponwhitelists = { 
    ...
    {name = "AN-PQ 15", gunhash = "WEAPON_PQ15HVC", permid = 1, price = 850000},

}



RegisterServerEvent("HVC:largeBuyWapwl")
AddEventHandler(
    "HVC:largeBuyWapwl",
    function(price, hash)
        local source = source
        ...
        if #(coords - comparison) > 20 then
            TriggerClientEvent("HVC:DEEEATH", source, "Tried hacking large with distance: " .. #(coords - comparison))
            return
        end
        local hash = hash
...
RegisterServerEvent("HVC:PULLWHITELISTEDWEAPONSHVCLARGE")
AddEventHandler("HVC:PULLWHITELISTEDWEAPONSHVCLARGE", function()
    local source = source
    local table = {}
...
Wait(300)
TriggerClientEvent("HVC:GUNSRETURNEDHVCLARGE", source,table)
end)
]]