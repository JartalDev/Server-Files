...
local playersinside = {}
local turfname = "Heroin Turf"
local taketurfpoint = vector3(3587.72, 3669.93, 33.88)
local chng = vector3(3586.77, 3665.37, 33.88)
...
--------------------------------
HVCserver = {}
Proxy.addInterface("heroin",HVCserver)

function HVCserver.getHeroinCom()
    return tonumber(turfcom)
end

function HVCserver.getHeroinOwnerN()
    return tostring(takengang)
end

function HVCserver.getHeroinOwner()
...
RegisterServerEvent("HVC:SetHeroinCommission")
AddEventHandler("HVC:SetHeroinCommission", function(com1)
    local source = source 
        ...
        if GangOwnerID ~= nil then
            ...
            if tostring(GangName) == takengang then 
                if turfcomsetcooldown < 0 then
                    ...
                    TriggerClientEvent("chatMessage", -1, "", {180, 0, 0}, "^7 Heroin Commission has been set to " .. tostring(turfcom) .. "% by " ..takengang, "alert")
                else 
...
RegisterServerEvent("HVC:EnterHeroinTurf")
AddEventHandler("HVC:EnterHeroinTurf", function()
    local source = source
    if playersinside[source] == nil then
        ...
        if turftakinggangname ~= 0 then
            if tostring(GangName) == tostring(turftakinggangname) then
                gang = gang + 1
              -- print("[HVC] Gang Member Entered Heroin Turf " ..GetPlayerName(source))
                --print(GangOwnerID, GangName, turftaker)
            end
            if tostring(GangName) ~= tostring(turftakinggangname)  then
                --print(GangOwnerID, GangName, turftaker)
              -- print("[HVC] Enemy Entered Heroin Turf "  ..GetPlayerName(source))
                enemy = true
                enemies = enemies + 1
            end
        end
...
RegisterServerEvent("HVC:LeaveHeroinTurf")
AddEventHandler("HVC:LeaveHeroinTurf", function()
    local source = source
    if playersinside[source] ~= nil then
        local id = HVC.getUserId({source})
        local zz = 0 
        ...
        if turftakinggangname ~= 0 then
            if tostring(GangName) == tostring(turftakinggangname) then
                gang = gang - 1
              -- print("[HVC] Gang Member Left Heroin Turf " ..GetPlayerName(source))
                --print(GangOwnerID, GangName, turftaker)
            end
            if tostring(GangName) ~= tostring(turftakinggangname)  then
                --print(GangOwnerID, GangName, turftaker)
              -- print("[HVC] Enemy Left Heroin Turf "  ..GetPlayerName(source))
                enemy = true
                enemies = enemies - 1
            end
        end
      ...
      -- print("Enemies: " ..tostring(enemy), enemies)
        ...
        if gang <= 0 and beingtaken == true then
          ...
          -- print(enemy, enemies)
            ...
            TriggerClientEvent("HVC:SetOuterZoneHeroin", -1, 0)
            turftakerid = 0
            beingtaken = false
            turftakinggangid = 0
...
RegisterServerEvent("HVC:TakeHeroinTurf")
AddEventHandler("HVC:TakeHeroinTurf", function()
    local source = source 
    if beingtaken ~= true then
        local GangOwnerID, GangName, turftaker = getGangOwner(source)
        ...
        if GangOwnerID ~= nil or GangOwnerID ~= "" then
            ...
            TriggerClientEvent("HVC:SetOuterZoneHeroin", -1, turfdefaulttime)
...
RegisterServerEvent("HVC:RefreshHeroinTurf")
AddEventHandler("HVC:RefreshHeroinTurf", function()
    Update(source)
end)
