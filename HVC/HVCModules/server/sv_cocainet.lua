...
local playersinside = {}
local turfname = "Cocaine Turf"
local taketurfpoint = vector3(108.05, -1305.33, 28.77)
local chng = vector3(106.78, -1306.11, 28.77)
...
--------------------------------
HVCserver = {}
Proxy.addInterface("cocaine",HVCserver)

function HVCserver.getCocaineCom()
    return tonumber(turfcom)
end

function HVCserver.getCocaineOwnerN()
    return tostring(takengang)
end

function HVCserver.getCocaineOwner()
...
RegisterServerEvent("HVC:SetCocaineCommission")
AddEventHandler("HVC:SetCocaineCommission", function(com1)
    local source = source 
        ...
        if GangOwnerID ~= nil then
            ...
            if tostring(GangName) == takengang then 
                if turfcomsetcooldown < 0 then
                    ...
                    TriggerClientEvent("chatMessage", -1, "", {180, 0, 0}, "^7 Cocaine Commission has been set to " .. tostring(turfcom) .. "% by " ..takengang, "alert")
                else 
...
RegisterServerEvent("HVC:EnterCocaineTurf")
AddEventHandler("HVC:EnterCocaineTurf", function()
    local source = source
    if playersinside[source] == nil then
        ...
        if turftakinggangname ~= 0 then
            if tostring(GangName) == tostring(turftakinggangname) then
                gang = gang + 1
              -- print("[HVC] Gang Member Entered Cocaine Turf " ..GetPlayerName(source))
                --print(GangOwnerID, GangName, turftaker)
            end
            if tostring(GangName) ~= tostring(turftakinggangname)  then
                --print(GangOwnerID, GangName, turftaker)
              -- print("[HVC] Enemy Entered Cocaine Turf "  ..GetPlayerName(source))
                enemy = true
                enemies = enemies + 1
            end
        end
...
RegisterServerEvent("HVC:LeaveCocaineTurf")
AddEventHandler("HVC:LeaveCocaineTurf", function()
    local source = source
    if playersinside[source] ~= nil then
        local id = HVC.getUserId({source})
        local zz = 0 
        ...
        if turftakinggangname ~= 0 then
            if tostring(GangName) == tostring(turftakinggangname) then
                gang = gang - 1
              -- print("[HVC] Gang Member Left Cocaine Turf " ..GetPlayerName(source))
                --print(GangOwnerID, GangName, turftaker)
            end
            if tostring(GangName) ~= tostring(turftakinggangname)  then
                --print(GangOwnerID, GangName, turftaker)
              -- print("[HVC] Enemy Left Cocaine Turf "  ..GetPlayerName(source))
                enemy = true
                enemies = enemies - 1
            end
        end
      ...
      -- print("Enemies: " ..tostring(enemy), enemies)
        ...
        if gang <= 0 and beingtaken == true then
          ...
          -- print(enemy, enemies)
            ...
            TriggerClientEvent("HVC:SetOuterZoneCocaine", -1, 0)
            turftakerid = 0
            beingtaken = false
            turftakinggangid = 0
...
RegisterServerEvent("HVC:TakeCocaineTurf")
AddEventHandler("HVC:TakeCocaineTurf", function()
    local source = source 
    if beingtaken ~= true then
        local GangOwnerID, GangName, turftaker = getGangOwner(source)
        ...
        if GangOwnerID ~= nil or GangOwnerID ~= "" then
            ...
            TriggerClientEvent("HVC:SetOuterZoneCocaine", -1, turfdefaulttime)
...
RegisterServerEvent("HVC:RefreshCocaineTurf")
AddEventHandler("HVC:RefreshCocaineTurf", function()
    Update(source)
end)
