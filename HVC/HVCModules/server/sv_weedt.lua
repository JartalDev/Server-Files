...
local playersinside = {}
local turfname = "Weed Turf"
local taketurfpoint = vector3(339.73, -2042.78, 21.34)
local chng = vector3(337.5, -2045.47, 21.17)
...
--------------------------------
HVCserver = {}
Proxy.addInterface("weed",HVCserver)

function HVCserver.getWeedCom()
    return tonumber(turfcom)
end

function HVCserver.getWeedOwnerN()
    return tostring(takengang)
end

function HVCserver.getWeedOwner()
...
RegisterServerEvent("HVC:SetWeedCommission")
AddEventHandler("HVC:SetWeedCommission", function(com1)
    local source = source 
    ...
    if #(Player - chng) < 2 then 
        ...
        if GangOwnerID ~= nil then
            ...
            if GangOwnerID == turftakerid then 
                if turfcomsetcooldown < 0 then
                    ...
                    TriggerClientEvent("chatMessage", -1, "", {180, 0, 0}, "^7 Weed Commission has been set to " .. tostring(turfcom) .. "% by " ..takengang, "alert")
                else 
...
RegisterServerEvent("HVC:EnterWeedTurf")
AddEventHandler("HVC:EnterWeedTurf", function()
    local source = source
...
RegisterServerEvent("HVC:LeaveWeedTurf")
AddEventHandler("HVC:LeaveWeedTurf", function()
    local source = source
    local enemies = 0
    local gang = 0
    if playersinside[source] ~= nil then
        ...
        for i,v in pairs(playersinside) do
            ...
            if tonumber(GangOwnerID) == tonumber(turftakerid) then
                gang = gang + 1
            end
            if GangOwnerID ~= turftakerid  then
            enemy = true
        ...
        if gang <= 0 and beingtaken == true then
            TriggerClientEvent("chatMessage", -1, "^1^*HVC", {180, 0, 0}, takengang .. " has failed to take the " .. turfname, "alert")
            TriggerClientEvent("HVC:SetOuterZoneWeed", -1, 0)
            turftakerid = 0
            beingtaken = false
            for i,v in pairs(playersinside) do
                local GangOwnerID, GangName, turftaker = getGangOwner(i)
...
RegisterServerEvent("HVC:TakeWeedTurf")
AddEventHandler("HVC:TakeWeedTurf", function()
    local source = source 
    if beingtaken ~= true then
        local GangOwnerID, GangName, turftaker = getGangOwner(source)
        ...
        if GangOwnerID ~= nil or GangOwnerID ~= "" then
            ...
            TriggerClientEvent("HVC:SetOuterZoneWeed", -1, turfdefaulttime)
...
RegisterServerEvent("HVC:RefreshWeedTurf")
AddEventHandler("HVC:RefreshWeedTurf", function()
    Update(source)
end)
        ...
        if enemy == true then
            for i,v in pairs(playersinside) do
                ...
                if turftakerid == GangOwnerID and tonumber(turftakerid) ~= 0 then
                    HVCclient.notify(source, {"Enemies in zone"})
                end
