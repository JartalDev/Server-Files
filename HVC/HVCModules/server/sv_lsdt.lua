...
local playersinside = {}
local turfname = "LSD Turf"
local taketurfpoint = vector3(2486.05, -428.66, 92.99)
local chng = vector3(2488.72, -430.37, 92.99)
...
--------------------------------
HVCserver = {}
Proxy.addInterface("lsd",HVCserver)

function HVCserver.getLSDCom()
    return tonumber(turfcom)
end

function HVCserver.getLSDOwnerN()
    return tostring(takengang)
end

function HVCserver.getLSDOwner()
...
RegisterServerEvent("HVC:SetLSDCommission")
AddEventHandler("HVC:SetLSDCommission", function(com1)
    local source = source 
        ...
        if GangOwnerID ~= nil then
            ...
            if tostring(GangName) == takengang then 
                if turfcomsetcooldown < 0 then
                    ...
                    TriggerClientEvent("chatMessage", -1, "", {180, 0, 0}, "^7 LSD Commission has been set to " .. tostring(turfcom) .. "% by " ..takengang, "alert")
                else 
...
RegisterServerEvent("HVC:EnterLSDTurf")
AddEventHandler("HVC:EnterLSDTurf", function()
    local source = source
    if playersinside[source] == nil then
        ...
        if turftakinggangname ~= 0 then
            if tostring(GangName) == tostring(turftakinggangname) then
                gang = gang + 1
              -- print("[HVC] Gang Member Entered LSD Turf " ..GetPlayerName(source))
                --print(GangOwnerID, GangName, turftaker)
            end
            if tostring(GangName) ~= tostring(turftakinggangname)  then
                --print(GangOwnerID, GangName, turftaker)
              -- print("[HVC] Enemy Entered LSD Turf "  ..GetPlayerName(source))
                enemy = true
                enemies = enemies + 1
            end
        end
...
RegisterServerEvent("HVC:LeaveLSDTurf")
AddEventHandler("HVC:LeaveLSDTurf", function()
    local source = source
    if playersinside[source] ~= nil then
        local id = HVC.getUserId({source})
        local zz = 0 
        ...
        if turftakinggangname ~= 0 then
            if tostring(GangName) == tostring(turftakinggangname) then
                gang = gang - 1
              -- print("[HVC] Gang Member Left LSD Turf " ..GetPlayerName(source))
                --print(GangOwnerID, GangName, turftaker)
            end
            if tostring(GangName) ~= tostring(turftakinggangname)  then
                --print(GangOwnerID, GangName, turftaker)
              -- print("[HVC] Enemy Left LSD Turf "  ..GetPlayerName(source))
                enemy = true
                enemies = enemies - 1
            end
        end
      ...
      -- print("Enemies: " ..tostring(enemy), enemies)
        ...
        if gang <= 0 and beingtaken == true then
          ...
          -- print(enemy, enemies)
            ...
            TriggerClientEvent("HVC:SetOuterZoneLSD", -1, 0)
            turftakerid = 0
            beingtaken = false
            turftakinggangid = 0
...
RegisterServerEvent("HVC:TakeLSDTurf")
AddEventHandler("HVC:TakeLSDTurf", function()
    local source = source 
    if beingtaken ~= true then
        local GangOwnerID, GangName, turftaker = getGangOwner(source)
        ...
        if GangOwnerID ~= nil or GangOwnerID ~= "" then
            ...
            TriggerClientEvent("HVC:SetOuterZoneLSD", -1, turfdefaulttime)
...
RegisterServerEvent("HVC:RefreshLSDTurf")
AddEventHandler("HVC:RefreshLSDTurf", function()
    Update(source)
end)
