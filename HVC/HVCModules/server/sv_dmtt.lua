...
local playersinside = {}
local turfname = "DMT Turf"
local taketurfpoint = vector3(-583.59, -1602.3, 27.01)
local chng = vector3(-589.74, -1603.1, 27.01)
...
--------------------------------
HVCserver = {}
Proxy.addInterface("dmt",HVCserver)

function HVCserver.getDMTCom()
    return tonumber(turfcom)
end

function HVCserver.getDMTOwnerN()
    return tostring(takengang)
end

function HVCserver.getDMTOwner()
...
RegisterServerEvent("HVC:SetDMTCommission")
AddEventHandler("HVC:SetDMTCommission", function(com1)
    local source = source 
        ...
        if GangOwnerID ~= nil then
            ...
            if tostring(GangName) == takengang then 
                if turfcomsetcooldown < 0 then
                    ...
                    TriggerClientEvent("chatMessage", -1, "", {180, 0, 0}, "^7 DMT Commission has been set to " .. tostring(turfcom) .. "% by " ..takengang, "alert")
                else 
...
RegisterServerEvent("HVC:EnterDMTTurf")
AddEventHandler("HVC:EnterDMTTurf", function()
    local source = source
    if playersinside[source] == nil then
        ...
        if turftakinggangname ~= 0 then
            if tostring(GangName) == tostring(turftakinggangname) then
                gang = gang + 1
              -- print("[HVC] Gang Member Entered DMT Turf " ..GetPlayerName(source))
                --print(GangOwnerID, GangName, turftaker)
            end
            if tostring(GangName) ~= tostring(turftakinggangname)  then
                --print(GangOwnerID, GangName, turftaker)
              -- print("[HVC] Enemy Entered DMT Turf "  ..GetPlayerName(source))
                enemy = true
                enemies = enemies + 1
            end
        end
...
RegisterServerEvent("HVC:LeaveDMTTurf")
AddEventHandler("HVC:LeaveDMTTurf", function()
    local source = source
    if playersinside[source] ~= nil then
        local id = HVC.getUserId({source})
        local zz = 0 
        ...
        if turftakinggangname ~= 0 then
            if tostring(GangName) == tostring(turftakinggangname) then
                gang = gang - 1
              -- print("[HVC] Gang Member Left DMT Turf " ..GetPlayerName(source))
                --print(GangOwnerID, GangName, turftaker)
            end
            if tostring(GangName) ~= tostring(turftakinggangname)  then
                --print(GangOwnerID, GangName, turftaker)
              -- print("[HVC] Enemy Left DMT Turf "  ..GetPlayerName(source))
                enemy = true
                enemies = enemies - 1
            end
        end
      ...
      -- print("Enemies: " ..tostring(enemy), enemies)
        ...
        if gang <= 0 and beingtaken == true then
          ...
          -- print(enemy, enemies)
            ...
            TriggerClientEvent("HVC:SetOuterZoneDMT", -1, 0)
            turftakerid = 0
            beingtaken = false
            turftakinggangid = 0
...
RegisterServerEvent("HVC:TakeDMTTurf")
AddEventHandler("HVC:TakeDMTTurf", function()
    local source = source 
    if beingtaken ~= true then
        local GangOwnerID, GangName, turftaker = getGangOwner(source)
        ...
        if GangOwnerID ~= nil or GangOwnerID ~= "" then
            ...
            TriggerClientEvent("HVC:SetOuterZoneDMT", -1, turfdefaulttime)
...
RegisterServerEvent("HVC:RefreshDMTTurf")
AddEventHandler("HVC:RefreshDMTTurf", function()
    Update(source)
end)
