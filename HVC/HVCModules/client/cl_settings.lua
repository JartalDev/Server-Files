...
RMenu.Add('SettingsMenu', 'MainMenu', RageUI.CreateMenu("", "Settings Menu", 1300,100, "banners","settings")) 
...
RageUI.CreateWhile(1.0, RMenu:Get('SettingsMenu', 'MainMenu'), nil, function()
    RageUI.IsVisible(RMenu:Get('SettingsMenu', 'MainMenu'), true, false, true, function()
        ...
        RageUI.List("Render Distance Modifier", d, dts, "~w~Change your render distance;\n~y~Lowering this will increase FPS.", {}, true, function(a,b,c,d)
            if c then
        ...
        RageUI.Checkbox("Toggle Compass","~w~Toggle compass to display on/off", compasschecked, {RightLabel = ""}, function(Hovered, Active, Selected, Checked)
            if (Selected) then
        ...
        RageUI.Checkbox("Toggle Streetnames","~w~Toggle streetnames to display on/off", streetnamechecked, {RightLabel = ""}, function(Hovered, Active, Selected, Checked)
            if (Selected) then
        ...
        RageUI.Checkbox("Toggle HUD","~w~Toggle your HUD UI!", hudchecked, {RightLabel = ""}, function(Hovered, Active, Selected, Checked)
            if (Selected) then
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('SettingsMenu', 'hitsoundsmenu')) then
        ...
        RageUI.Checkbox("Enable Crosshair | Press [E] To Save", nil, crosshairchecked, {RightLabel = ""}, function(Hovered, Active, Selected, Checked)
            if (Selected) then
        ...
        local function ToggleUITrue()
            -- Show IDs/HUD
            TriggerEvent("HVC-Client:UI:Toggle", true)
            TriggerEvent("HVC-Client:IDS:Toggle", true)
            UI = true
        end
        local function ToggleUIFalse()
            TriggerEvent("HVC-Client:UI:Toggle", false)
            TriggerEvent("HVC-Client:IDS:Toggle", false)
            UI = false
        end
        RageUI.Checkbox("Toggle Money UI","~w~Toggle your money UI!", UI, {Style = RageUI.CheckboxStyle.Car}, function(Hovered, Active, Selected, Checked)

        end, ToggleUITrue, ToggleUIFalse)
        ...
        RageUI.Checkbox("Toggle Killfeed UI","~w~Toggle your killfeed UI!", Killfeed, {Style = RageUI.CheckboxStyle.Car}, function(Hovered, Active, Selected, Checked)
