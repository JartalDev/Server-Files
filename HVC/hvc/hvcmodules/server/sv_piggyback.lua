...
AddEventHandler("Piggyback:sync", function(targetSrc)
	...
	if #(sourceCoords - targetCoords) <= 3.0 then 
		TriggerClientEvent("HVC:ClientpiggyRequestReceive", targetSrc)
...
RegisterServerEvent("HVC:PiggyServerValidEmote") 
AddEventHandler("HVC:PiggyServerValidEmote", function(target)
    TriggerClientEvent("Piggyback:syncTarget", source, target)
    TriggerClientEvent("HVC:SyncPiggying", target, source)
end)