  ...
  if user_id ~= nil then
    ...
    if data then
      if data.cloakroom_idle ~= nil then -- consume cloakroom idle
        HVCclient.setCustomization(player,{data.cloakroom_idle})
  ...
  local choose = function(player, choice)
    ...
    if custom then
      HVCclient.getCustomization(player,{},function(custom)
        ...
        HVCclient.setCustomization(player,{idle_copy})
      end)
    end
  end
...
local function build_client_points(source)
  for k,v in pairs(cfg.cloakrooms) do
    ...
    if cloakroom and menu then
      ...
      HVC.setArea(source,"HVC:cfg:cloakroom"..k,x,y,z,1,1.5,cloakroom_enter,cloakroom_leave)
    end
  end
end
