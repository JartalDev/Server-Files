...
-- name = partid
function HVC.openSkinshop(source, parts)
    ...
    if user_id ~= nil then
        ...
        if data.cloakroom_idle ~= nil then
            HVCclient.notify(source, {lang.common.wearing_uniform()})
        end

        -- get old customization to compute the price
        HVCclient.getCustomization(source, {}, function(old_custom)
            ...
            local ontexture = function(player, choice)
                ...
                HVCclient.setCustomization(source, {custom})
            end
            ...
            local ondrawable = function(player, choice, mod)
                if mod == 0 then -- tex variation
                    ...
                    HVCclient.getDrawableTextures(source, {parts[choice], drawable[1]}, function(n)
                        print(n)
                        textures[choice][2] = n
            ...
            for k, v in pairs(parts) do -- for each part, get number of drawables and build menu
                ...
                -- get max drawables
                HVCclient.getDrawables(source, {v}, function(n)
                    drawables[k][2] = n -- set max
                   
                end)

                -- get max textures for this drawable
                HVCclient.getDrawableTextures(source, {v, drawables[k][1]}, function(n)
            ...
            menudata.onclose = function(player)
                -- compute price
                HVCclient.getCustomization(source, {}, function(custom)
                    local price = 0
                    ...
                    if HVC.tryPayment(user_id, price) then
                        if price > 0 then
                            HVCclient.notify(source, {lang.money.paid({price})})
                        end
                    else
                        HVCclient.notify(source, {lang.money.not_enough()})
                        -- revert changes
                        HVCclient.setCustomization(source, {old_custom})
                    end
                end)
            end
            -- open menu
...
local function build_client_skinshops(source)
    ...
    if user_id ~= nil then
        for k, v in pairs(skinshops) do
            ...
            HVCclient.addBlip(source, {x, y, z, 73, 3, lang.skinshop.title()})
            -- HVCclient.addMarker(source, {x, y, z - 1, 0.7, 0.7, 0.5, 0, 255, 125, 125, 150})

            HVC.setArea(source, "HVC:skinshop" .. k, x, y, z, 1, 1.5, skinshop_enter, skinshop_leave)
        end
    end
end
