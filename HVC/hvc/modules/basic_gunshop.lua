  ...
  local gunshop_choice = function(player,choice)
   ...
   if weapon then
      ...
      HVCclient.getWeapons(player,{},function(weapons)
        -- prompt amount
        HVC.prompt(player,lang.gunshop.prompt_ammo({choice}),"",function(player,amount)
          local amount = parseInt(amount)
			...
			-- payment
			if user_id ~= nil and HVC.tryPayment(user_id,total) then
			  HVCclient.giveWeapons(player,{{
				[weapon] = {ammo=amount}
			  }})

			  HVCclient.notify(player,{lang.money.paid({total})})
			else
  ...
  if user_id ~= nil then
    for k,v in pairs(gunshops) do
      ...
      if group and menu then
        ...
        HVCclient.addBlip(source,{x,y,z,gcfg.blipid,gcfg.blipcolor,lang.gunshop.title({gtype})})
        -- HVCclient.addMarker(source,{x,y,z-1,0.7,0.7,0.5,0,255,125,125,150})

        HVC.setArea(source,"HVC:gunshop"..k,x,y,z,1,1.5,gunshop_enter,gunshop_leave)
      end
    end
  end
end
