...
function HVC.openMenu(source,menudef)
  local menudata = {}
  menudata.choices = {}
  ...
  -- openmenu
  HVCclient.openMenuData(source,{menudata})
end

-- force close player menu
function HVC.closeMenu(source)
  HVCclient.closeMenu(source,{})
end
...
function HVC.prompt(source,title,default_text,cb_result)
  prompts[source] = cb_result

  HVCclient.prompt(source,{title,default_text})
end
...
function HVC.request(source,text,time,cb_ok)
  local id = request_ids:gen()
  local request = {source = source, cb_ok = cb_ok, done = false}
  requests[id] = request

  HVCclient.request(source,{id,text,time}) -- send request to client
...
function HVC.registerMenuBuilder(name, builder)
...
function HVC.buildMenu(name, data, cbr)
...
function HVC.openMainMenu(source)
  HVC.buildMenu("main", {player = source}, function(menudata)
    menudata.name = "Main menu"
    menudata.css = {top="75px",header_color="rgba(0,125,255,0.75)"}
    HVC.openMenu(source,menudata) -- open the generated menu
  end)
end

-- SERVER TUNNEL API

function tHVC.closeMenu(id)
...
function tHVC.validMenuChoice(id,choice,mod)
  local menu = client_menus[id]
  if menu and menu.source == source then
...
function tHVC.promptResult(text)
  if text == nil then
    text = ""
  end
...
function tHVC.requestResult(id,ok)
...
function tHVC.openMainMenu()
  HVC.openMainMenu(source)
end
...
function HVC.addStaticMenuChoices(name, choices)
...
-- build static menus
local static_menus = {}
SetTimeout(10000,function() -- wait for HVC.addStaticMenuChoices calls
    ...
    for k,v in pairs(cfg.static_menus) do
      ...
      if menu and smenu then
        local function smenu_enter()
          local user_id = HVC.getUserId(source)
          if user_id ~= nil and HVC.hasPermissions(user_id,smenu.permissions or {}) then
        ...
        HVCclient.addBlip(source,{x,y,z,smenu.blipid,smenu.blipcolor,smenu.title})
        -- HVCclient.addMarker(source,{x,y,z-1,0.7,0.7,0.5,255,226,0,125,150})

        HVC.setArea(source,"HVC:static_menu:"..k,x,y,z,1,1.5,smenu_enter,smenu_leave)
      end
    end
  end
end
...
AddEventHandler("HVC:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    HVCclient.setDiv(source,{"additional_css",".div_additional_css{ display: none; }\n\n"..cfg.css,""})
