...
MySQL.createCommand("HVC/get_address","SELECT home, number FROM hvc_user_homes WHERE user_id = @user_id")
MySQL.createCommand("HVC/get_home_owner","SELECT user_id FROM hvc_user_homes WHERE home = @home AND number = @number")
MySQL.createCommand("HVC/rm_address","DELETE FROM hvc_user_homes WHERE user_id = @user_id")
MySQL.createCommand("HVC/set_address","REPLACE INTO hvc_user_homes(user_id,home,number) VALUES(@user_id,@home,@number)")
...
function HVC.getUserAddress(user_id, cbr)
  local task = Task(cbr)

  MySQL.query("HVC/get_address", {user_id = user_id}, function(rows,affected)
    task({rows[1]})
  end)
end

-- set user address
function HVC.setUserAddress(user_id,home,number)
  MySQL.execute("HVC/set_address", {user_id = user_id, home = home, number = number})
end

-- remove user address
function HVC.removeUserAddress(user_id)
  MySQL.execute("HVC/rm_address", {user_id = user_id})
end

-- cbreturn user_id or nil
function HVC.getUserByAddress(home,number,cbr)
  local task = Task(cbr)

  MySQL.query("HVC/get_home_owner", {home = home, number = number}, function(rows, affected)
    if #rows > 0 then
      task({rows[1].user_id})
    else
      task()
...
function HVC.findFreeNumber(home,max,cbr)
  local task = Task(cbr)

  local i = 1
  local function search()
    HVC.getUserByAddress(home,i,function(user_id)
...
function HVC.defHomeComponent(name, oncreate, ondestroy)
...
local function leave_slot(user_id,player,stype,sid) -- called when a player leave a slot
  ...
  HVCclient.teleport(player, home.entry_point) -- already an array of params (x,y,z)
  ...
  for k,v in pairs(cfg.slot_types[stype][sid]) do
    ...
    if name == "entry" then
      -- remove marker/area
      local nid = "HVC:home:slot"..stype..sid
  ...
  HVC.getUserAddress(user_id, function(address)
    ...
    for k,v in pairs(cfg.slot_types[stype][sid]) do
      ...
      if name == "entry" then
        ...
        HVCclient.teleport(player, {x,y,z}) -- already an array of params (x,y,z)

        local nid = "HVC:home:slot"..stype..sid
        HVCclient.setNamedMarker(player,{nid,x,y,z-1,0.7,0.7,0.5,0,255,125,125,150})
        HVC.setArea(player,nid,x,y,z,1,1.5,entry_enter,entry_leave)
...
function HVC.accessHome(user_id, home, number, cbr)
  local task = Task(cbr)
  ...
  HVC.getUserByAddress(home,number, function(owner_id)
...
local function build_entry_menu(user_id, home_name)
  ...
  menu[lang.home.intercom.title()] = {function(player,choice)
    HVC.prompt(player, lang.home.intercom.prompt(), "", function(player,number)
      number = parseInt(number)
      HVC.getUserByAddress(home_name,number,function(huser_id)
        if huser_id ~= nil then
          if huser_id == user_id then -- identify owner (direct home access)
            HVC.accessHome(user_id, home_name, number, function(ok)
              if not ok then
                HVCclient.notify(player,{lang.home.intercom.not_available()})
              end
            end)
          else -- try to access home by asking owner
            local hplayer = HVC.getUserSource(huser_id)
            if hplayer ~= nil then
              HVC.prompt(player,lang.home.intercom.prompt_who(),"",function(player,who)
                HVCclient.notify(player,{lang.home.intercom.asked()})
                -- request owner to open the door
                HVC.request(hplayer, lang.home.intercom.request({who}), 30, function(hplayer,ok)
                  if ok then
                    HVC.accessHome(user_id, home_name, number)
                  else
                    HVCclient.notify(player,{lang.home.intercom.refused()})
                  end
                end)
              end)
            else
  ...
  menu[lang.home.buy.title()] = {function(player,choice)
    HVC.getUserAddress(user_id, function(address)
      if address == nil then -- check if not already have a home
        HVC.findFreeNumber(home_name, home.max, function(number)
          if number ~= nil then
            if HVC.tryPayment(user_id, home.buy_price) then
              -- bought, set address
              HVC.setUserAddress(user_id, home_name, number)

              HVCclient.notify(player,{lang.home.buy.bought()})
            else
              HVCclient.notify(player,{lang.money.not_enough()})
            end
          else
            HVCclient.notify(player,{lang.home.buy.full()})
          end
        end)
      else
        HVCclient.notify(player,{lang.home.buy.have_home()})
      end
    end)
  end, lang.home.buy.description({home.buy_price})}

  menu[lang.home.sell.title()] = {function(player,choice)
   HVC.request(player,"Are you sure that you want to sell your house?",30,function(player,ok)
     if ok then
        HVC.getUserAddress(user_id, function(address)
          if address ~= nil and address.home == home_name then -- check if already have a home
            -- sold, give sell price, remove address
            HVC.giveMoney(user_id, home.sell_price)
            HVC.removeUserAddress(user_id)
            HVCclient.notify(player,{lang.home.sell.sold()})
          else
            HVCclient.notify(player,{lang.home.sell.no_home()})
          end
        end)
      end
    end)
  ...
  if user_id ~= nil then
    for k,v in pairs(cfg.homes) do
      ...
      local function entry_enter(player,area)
        ...
        if user_id ~= nil and HVC.hasPermissions(user_id,v.permissions or {}) then
          HVC.openMenu(source,build_entry_menu(user_id, k))
        end
      end
      ...
      HVCclient.addBlip(source,{x,y,z,v.blipid,v.blipcolor,houseName})
      -- HVCclient.addMarker(source,{x,y,z-1,0.7,0.7,0.5,0,255,125,125,150})

      HVC.setArea(source,"HVC:home"..k,x,y,z,1,1.5,entry_enter,entry_leave)
    end
  end
end
