...
local function tr_remove_player(tr,player) -- remove player from transforming
  ...
  HVCclient.removeProgressBar(player,{"HVC:tr:"..tr.name})
...
local function tr_add_player(tr,player,recipe) -- add player to transforming
  ...
  HVC.closeMenu(player)
  HVCclient.setProgressBar(player,{"HVC:tr:"..tr.name,"center",recipe.."...",tr.itemtr.r,tr.itemtr.g,tr.itemtr.b,0})
...
local function tr_tick(tr) -- do transformer tick
  for k,v in pairs(tr.players) do
    local user_id = HVC.getUserId(tonumber(k))
    local player = HVC.getUserSource(tonumber(user_id))
    if v and user_id ~= nil then -- for each player transforming
      
     HVCclient.getPosition(player,{},function(x,y,z)
	    HVCclient.getDistanceBetweenCoords(player,{x,y,z,tr.itemtr.x,tr.itemtr.y,tr.itemtr.z},function(distance)
      ...
      if tr.units > 0 and recipe then -- check units
        ...
        for l,w in pairs(recipe.reagents) do
          reagents_ok = reagents_ok and (HVC.getInventoryItemAmount(user_id,l) >= w)
        end

        -- check money
        local money_ok = (HVC.getMoney(user_id) >= recipe.in_money)
        ...
        local new_weight = HVC.getInventoryWeight(user_id)+HVC.computeItemsWeight(out_witems)-HVC.computeItemsWeight(in_witems)

        local inventory_ok = true
        if new_weight > HVC.getInventoryMaxWeight(user_id) then
          inventory_ok = false
          HVCclient.notify(tonumber(k), {lang.inventory.full()})
        end
        ...
        if money_ok and reagents_ok and inventory_ok then -- do transformation
          ...
          -- consume reagents
          if recipe.in_money > 0 then HVC.tryPayment(user_id,recipe.in_money) end
          for l,w in pairs(recipe.reagents) do
            HVC.tryGetInventoryItem(user_id,l,w,true)
          end

          -- produce products
          if recipe.out_money > 0 then HVC.giveMoney(user_id,recipe.out_money) end
          for l,w in pairs(recipe.products) do
            HVC.giveInventoryItem(user_id,l,w,true)
          end
          ...
          for l,w in pairs(recipe.aptitudes or {}) do
            ...
            if #parts == 2 then
              HVC.varyExp(user_id,parts[1],parts[2],w)
            end
          end
  ...
  for k,v in pairs(tr.players) do
    HVCclient.setProgressBarValue(k,{"HVC:tr:"..tr.name,math.floor(tr.units/tr.itemtr.max_units*100.0)})
    
    if tr.units > 0 then -- display units left
      HVCclient.setProgressBarText(k,{"HVC:tr:"..tr.name,v.."... "..tr.units.."/"..tr.itemtr.max_units})
    else
      HVCclient.setProgressBarText(k,{"HVC:tr:"..tr.name,"empty"})
    end
  end
end

local function bind_tr_area(player,tr) -- add tr area to client
  HVC.setArea(player,"HVC:tr:"..tr.name,tr.itemtr.x,tr.itemtr.y,tr.itemtr.z,tr.itemtr.radius,tr.itemtr.height,tr.enter,tr.leave)
end

local function unbind_tr_area(player,tr) -- remove tr area from client
  HVC.removeArea(player,"HVC:tr:"..tr.name)
end
...
function HVC.setItemTransformer(name,itemtr)
  HVC.removeItemTransformer(name) -- remove pre-existing transformer
  ...
  for action,recipe in pairs(tr.itemtr.recipes) do
    ...
    for k,v in pairs(recipe.aptitudes or {}) do
      ...
      if #parts == 2 then
        local def = HVC.getAptitudeDefinition(parts[1],parts[2])
        if def then
          info = info.."<br />[EXP] "..v.." "..HVC.getAptitudeGroupTitle(parts[1]).."/"..def[1]
        end
      end
    end
  ...
  tr.enter = function(player,area)
    ...
    if user_id ~= nil and HVC.hasPermissions(user_id,itemtr.permissions or {}) then
      HVC.openMenu(player, tr.menu) -- open menu
    end
  end
  ...
  for k,v in pairs(HVC.rusers) do
    local source = HVC.getUserSource(k)
    if source ~= nil then
...
function HVC.removeItemTransformer(name)
...
SetTimeout(5000,function()
  ...
  for k,v in pairs(cfg.item_transformers) do
    HVC.setItemTransformer("cfg:"..k,v)
  end
end)
...
local function hidden_placement_tick()
  HVC.getSData("HVC:hidden_trs", function(data)
    ...
    for k,v in pairs(cfg.hidden_transformers) do
      ...
      if tonumber(os.time())-htr.timestamp >= cfg.hidden_transformer_duration*60 then
        ...
        HVC.removeItemTransformer("cfg:"..k)
      ...
      if transformers["cfg:"..k] == nil then
        ...
        HVC.setItemTransformer("cfg:"..k, v.def)
      end
    end

    HVC.setSData("HVC:hidden_trs",json.encode(hidden_trs)) -- save hidden transformers
  end)
...
local function ch_informer_buy(player,choice)
  ...
  if user_id ~= nil and tr ~= nil then
    if HVC.tryPayment(user_id, price) then
      HVCclient.setGPS(player, {tr.itemtr.x,tr.itemtr.y}) -- set gps marker
      HVCclient.notify(player, {lang.money.paid({price})})
      HVCclient.notify(player, {lang.itemtr.informer.bought()})
    else
...
local function informer_enter()
  ...
  if user_id ~= nil then
    HVC.openMenu(source,informer_menu) 
  end
end
...
local function informer_placement_tick()
  ...
  for k,v in pairs(HVC.rusers) do
    ...
    --HVCclient.setNamedBlip(player,{"HVC:informer",x,y,z,cfg.informer.blipid,cfg.informer.blipcolor,lang.itemtr.informer.title()})
    --HVCclient.setNamedMarker(player,{"HVC:informer",x,y,z-1,0.7,0.7,0.5,0,255,125,125,150})
    --HVC.setArea(player,"HVC:informer",x,y,z,1,1.5,informer_enter,informer_leave)
  end
  ...
  SetTimeout(cfg.informer.duration*60000, function()
    for k,v in pairs(HVC.rusers) do
      local player = HVC.getUserSource(tonumber(k))
      HVCclient.removeNamedBlip(player,{"HVC:informer"})
      HVCclient.removeNamedMarker(player,{"HVC:informer"})
      HVC.removeArea(player,"HVC:informer")
    end
  end)
