...
function HVC.getUserGroups(user_id)
...
function HVC.addUserGroup(user_id,group)
  if not HVC.hasGroup(user_id,group) then
    local user_groups = HVC.getUserGroups(user_id)
    ...
    if ngroup then
      ...
      TriggerEvent("HVC:playerJoinGroup", user_id, group, gtype)
    end
  end
end
...
function HVC.getUserGroupByType(user_id,gtype)
  local user_groups = HVC.getUserGroups(user_id)
  for k,v in pairs(user_groups) do
    local kgroup = groups[k]
    if kgroup then
...
function HVC.getUsersByGroup(group)
  local users = {}

  for k,v in pairs(HVC.rusers) do
    if HVC.hasGroup(tonumber(k),group) then table.insert(users, tonumber(k)) end
  end
...
function HVC.getUsersByPermission(perm)
  local users = {}

  for k,v in pairs(HVC.rusers) do
    if HVC.hasPermission(tonumber(k),perm) then table.insert(users, tonumber(k)) end
  end
...
function HVC.removeUserGroup(user_id,group)
  local user_groups = HVC.getUserGroups(user_id)
  ...
  TriggerEvent("HVC:playerLeaveGroup", user_id, group, gtype)

  user_groups[group] = nil -- remove reference
end

-- check if the user has a specific group
function HVC.hasGroup(user_id,group)
  local user_groups = HVC.getUserGroups(user_id)
  return (user_groups[group] ~= nil)
end

-- check if the user has a specific permission
function HVC.hasPermission(user_id, perm)
  local user_groups = HVC.getUserGroups(user_id)
  ...
  if fchar == "@" then -- special aptitude permission
    ...
    if #parts == 3 then -- decompose group.aptitude.operator
      ...
      local alvl = math.floor(HVC.expToLevel(HVC.getExp(user_id,group,aptitude)))
  ...
  elseif fchar == "#" then -- special item permission
    ...
    if #parts == 2 then -- decompose item.operator
      ...
      local amount = HVC.getInventoryItemAmount(user_id, item)
...
function HVC.hasPermissions(user_id, perms)
...
local function ch_select(player,choice)
  ...
  if user_id ~= nil then
    HVC.addUserGroup(user_id, choice)
  ...
  if user_id ~= nil then
    for k,v in pairs(selectors) do
      ...
      if gcfg and menu then
        ...
        HVCclient.addBlip(source,{x,y,z,gcfg.blipid,gcfg.blipcolor,k})
        -- HVCclient.addMarker(source,{x,y,z-1,0.7,0.7,0.5,255,154,24,125,150})

        HVC.setArea(source,"HVC:gselector:"..k,x,y,z,1,1.5,selector_enter,selector_leave)
      end
    end
  end
end
  ...
  if first_spawn then
    ...
    if user ~= nil then
      for k,v in pairs(user) do
        HVC.addUserGroup(user_id,v)
      end
    end

    -- add default group user
    HVC.addUserGroup(user_id,"user")
  end
