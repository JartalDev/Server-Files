  ...
  for gtype,mitems in pairs(market_types) do
    ...
    local market_choice = function(player,choice)
      ...
      if item then
        ...
        if user_id ~= nil then
          HVC.prompt(player,lang.market.prompt({item.name}),"",function(player,amount)
            local amount = parseInt(amount)
            if amount > 0 then
              -- weight check
              local new_weight = HVC.getInventoryWeight(user_id)+item.weight*amount
              if new_weight <= HVC.getInventoryMaxWeight(user_id) then
                -- payment
                if HVC.tryPayment(user_id,amount*price) then
                  HVC.giveInventoryItem(user_id,idname,amount,true)
                  HVCclient.notify(player,{lang.money.paid({amount*price})})
                else
  ...
  if user_id ~= nil then
    for k,v in pairs(markets) do
      ...
      if group and menu then -- check market type
        ...
        HVCclient.addBlip(source,{x,y,z,gcfg.blipid,gcfg.blipcolor,lang.market.title({gtype})})
        -- HVCclient.addMarker(source,{x,y,z-1,0.7,0.7,0.5,0,255,125,125,150})

        HVC.setArea(source,"HVC:market"..k,x,y,z,1,1.5,market_enter,market_leave)
      end
    end
  end
end
