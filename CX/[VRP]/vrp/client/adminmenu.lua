...
RegisterNetEvent('CXRP:SendAdminPerks', function(a)
    ...
    if getStaffLevel() > 0 then 
        print('[CXRP] Your staff level is: ' ..Stafflevel)
...
RMenu.Add('SettingsMenu', 'MainMenu', RageUI.CreateSubMenu(RMenu:Get('adminmenu', 'main'), "", menuColour.."Settings Menu", GetRageUIMenuWidth(),GetRageUIMenuHeight(), "new_editor","r_editor_header")) 
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('adminmenu', 'devfunctions')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.Separator("~g~Money Functions", "", {}, true,function(Hovered, Active, Selected)end)
                if admincfg.buttonsEnabled["devMenu"][1] and buttons["devMenu"] then
                ...
                if admincfg.buttonsEnabled["devMenu"][1] and buttons["devMenu"] then
                    RageUI.ButtonWithStyle("Give Bank","",{}, true, function(Hovered, Active, Selected)
                        if Selected then
                            TriggerServerEvent("CXRP:GiveBankMenu")
                        end
                    end, RMenu:Get('adminmenu', 'devfunctions'))
                end
...
--[[           if admincfg.buttonsEnabled["devMenu"][1] and buttons["devMenu"] then
                RageUI.ButtonWithStyle("~b~Other Functions", "", {RightLabel = "→"}, true, function(Hovered, Active, Selected)
                end, RMenu:Get('adminmenu', 'otherfunctions'))
            end   ]] 
        end)
    end
end)
...
--[[RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('adminmenu', 'otherfunctions')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.Separator("~b~Other Functions", "", {}, true,function(Hovered, Active, Selected)end)
            if admincfg.buttonsEnabled["devMenu"][1] and buttons["devMenu"] then
                RageUI.ButtonWithStyle("Reset Rewards","", {}, true, function(Hovered, Active, Selected)
                    if Selected then
...
function tvRP.setDeathSound(i)
    SetResourceKvp("vRP_deathsound",i)
end
function tvRP.getDeathSound()
    local k=GetResourceKvpString("vRP_deathsound")
