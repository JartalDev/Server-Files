...
RegisterCommand('stuck', function(source, args, rawCommand)
    TriggerEvent('CXRP:stuckTeleport')
end)

RegisterNetEvent('CXRP:stuckTeleport')
AddEventHandler('CXRP:stuckTeleport', function()
    --if not inRedZone then
        if stuckCooldown == false then
            notify("~g~You will be teleported to Legion in 3 minutes.")
            stuckCooldown = true
            Wait(60000*3)
            TriggerServerEvent("CXRP:Teleport", source, vector3(151.61740112305,-1035.05078125,29.339416503906))
            stuckCooldown = false
        end
