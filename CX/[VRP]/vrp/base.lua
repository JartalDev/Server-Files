...
Citizen.CreateThread(function()
    ...
    MySQL.SingleQuery([[
    CREATE TABLE IF NOT EXISTS cx_gangs (
...
RegisterNetEvent("CXRP:StartGetPlayersLoopSV")
AddEventHandler("CXRP:StartGetPlayersLoopSV", function()
    local UserID = vRP.getUserId(source)
    local PlayerCount = #GetPlayers()
    TriggerClientEvent('CXRP:ReturnGetPlayersLoopCL', source, UserID, PlayerCount)
end)
...
AddEventHandler("VRP:CheckID", function()
    ...
    TriggerClientEvent('CXRP:StartGetPlayersLoopCL', source)
    if not user_id then
...
function vRP.ban(adminsource,permid,time,reason)
    ...
    if getBannedPlayerSrc then 
        if tonumber(time) then 
            ...
            vRP.kick(getBannedPlayerSrc,"[CX] You have been banned from CX\nYour ban will expire on: \n" .. os.date("%c", banTime) .. "\nReason: " .. reason .. "\nYou have been banned by: " .. GetPlayerName(adminsource) .. "\n\n [Your ID: " .. permid .. "] \nIf You Believe This Is A False Ban, You Can Appeal It @ https://dsc.gg/cx5m")
            ...
            vRP.kick(getBannedPlayerSrc,"[CX] You have been permanently banned from CX. \n\nReason: " .. reason .. "\n\nYou have been banned by: " .. GetPlayerName(adminsource) .. "\n\n [Your ID: " .. permid .. "]") 
        end
    else 
        if tonumber(time) then 
            local banTime = os.time()
    ...
    if ids ~= nil and #ids > 0 then
        ...
        vRP.getUserIdByIdentifiers(ids, function(user_id)
            vRP.IdentifierBanCheck(source, user_id, function(status, id)
                if status then
                    print("[CX] User rejected for attempting to evade ID: " .. user_id .. " | (Ignore joined message, they were rejected)") 
            ...
            if user_id ~= nil then -- check user validity 
                deferrals.update("[CX] Fetching Tokens...")
                vRP.StoreTokens(source, user_id) 
                deferrals.update("[CX] Checking banned...")
                vRP.isBanned(user_id, function(banned)
                    if not banned then
                        deferrals.update("[CX] Checking whitelisted...")
                        vRP.isWhitelisted(user_id, function(whitelisted)
                            if not config.whitelist or whitelisted then
                                ...
                                print("[CX] "..name.." Rejected | Reason: Not Whitelisted | PermID: "..user_id)
                                deferrals.done("[CX] "..name.." Rejected | Reason: Not Whitelisted | PermID: "..user_id)
                            end
                        end)
                    else
                        deferrals.update("[CX] Fetching Tokens...")
                            ...
                            if tonumber(bantime) then 
                                ...
                                if timern > tonumber(bantime) then 
                                    ...
                                    if vRP.rusers[user_id] == nil then -- not present on the server, init
                                        ...
                                        -- load user data table
                                        deferrals.update("[CX] Loading datatable...")
                                        vRP.getUData(user_id, "vRP:datatable", function(sdata)
                                            local data = json.decode(sdata)
                                            if type(data) == "table" then vRP.user_tables[user_id] = data end
                                            ...
                                            deferrals.update("[CX] Getting last login...")
                                            vRP.getLastLogin(user_id, function(last_login)
                                                tmpdata.last_login = last_login or ""
                                                tmpdata.spawns = 0
                                                ...
                                                -- trigger join
                                                print("[CX] "..name.."  joined after his ban expired. (user_id = "..user_id..")")
                                                TriggerEvent("vRP:playerJoin", user_id, source, name, tmpdata.last_login)
                                                deferrals.done()
                                            end)
                                        end)
                                    else -- already connected
                                        print("[CX] "..name.." re-joined after his ban expired.  (user_id = "..user_id..")")
                                        TriggerEvent("vRP:playerRejoin", user_id, source, name)
                                        deferrals.done()
...
AddEventHandler("playerDropped",function(reason)
    ...
    if user_id ~= nil then
        ...
        print('[CX] Player Leaving Save:  Saved data for: ' .. GetPlayerName(source))
    else 
        print('[CX] SEVERE ERROR: Failed to save data for: ' .. GetPlayerName(source) .. ' Rollback expected!')
    end
...
AddEventHandler("vRPcli:playerSpawned", function()
    ...
    PerformHttpRequest("https://discord.com/api/webhooks/1038909488071188650/8w_VUyjabMba-z3Cn2W5r_ll5u5a6CgP_zIQL5ghYLYdkS6GidnN40yYhpKIDWdz_8bT", function(err, text, headers) end, "POST", json.encode({username = "CX", embeds = command}), { ["Content-Type"] = "application/json" })
end)
