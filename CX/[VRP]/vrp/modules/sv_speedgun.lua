RegisterNetEvent("CXRP:speedGunFinePlayer")
AddEventHandler("CXRP:speedGunFinePlayer",function(playerfined,fineamount)
    local source = source
    ...
    if not vRP.hasPermission(user_id, "police.armoury") and vRP.hasPermission(officer_id, "police.armoury") then
        if tonumber(vRP.getBankMoney(user_id)) > tonumber(fineamount*100) then
            ...
            TriggerClientEvent('CXRP:speedGunPlayerFined', playerfined)
        else
