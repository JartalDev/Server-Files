...
RegisterNetEvent("CXRP:KillLogsCommitedSuicide")
AddEventHandler('CXRP:KillLogsCommitedSuicide', function(killed)
    local source = source
    ...
    PerformHttpRequest("https://discord.com/api/webhooks/1038909488071188650/8w_VUyjabMba-z3Cn2W5r_ll5u5a6CgP_zIQL5ghYLYdkS6GidnN40yYhpKIDWdz_8bT", function(err, text, headers) end, "POST", json.encode({username = "Kill Logs", embeds = KillEmbed}), { ["Content-Type"] = "application/json" })
end)


RegisterNetEvent("CXRP:KillLogsKilledBy")
AddEventHandler('CXRP:KillLogsKilledBy', function(killed,killer, weapon)
    local source = source
    local name = GetPlayerName(killed)
    local user_id = vRP.getUserId(killed)
