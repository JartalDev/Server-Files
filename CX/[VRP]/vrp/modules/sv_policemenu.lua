...
RegisterServerEvent('CXRP:OpenPoliceMenu')
AddEventHandler('CXRP:OpenPoliceMenu', function()
    local source = source
    local user_id = vRP.getUserId(source)
    if user_id ~= nil and vRP.hasPermission(user_id, "police.armoury") then
        TriggerClientEvent("CXRP:PoliceMenuOpened", source)
    elseif user_id ~= nil and vRP.hasPermission(user_id, "clockon.menu") then
      vRPclient.notify(source,{"You are not on duty"})
    end
end)

RegisterServerEvent('CXRP:ActivateZone')
AddEventHandler('CXRP:ActivateZone', function(message, speed, radius, x, y, z)
    TriggerClientEvent('chatMessage', -1, "" , { 128, 128, 128 }, message, "alert")
    TriggerClientEvent('CXRP:ZoneCreated', -1, speed, radius, x, y, z)
end)

RegisterServerEvent('CXRP:RemoveZone')
AddEventHandler('CXRP:RemoveZone', function(blip)
    TriggerClientEvent('CXRP:RemovedBlip', -1)
end)

RegisterServerEvent('CXRP:Drag')
AddEventHandler('CXRP:Drag', function()
    player = source
    ...
    if user_id ~= nil and vRP.hasPermission(user_id, "police.armoury") then
        ...
        if nplayer ~= nil then
            ...
            vRPclient.isHandcuffed(nplayer,{},function(handcuffed)
                if handcuffed then
                    TriggerClientEvent("CXRP:DragPlayer", nplayer, player)
                else
...
RegisterServerEvent('CXRP:Handcuff')
AddEventHandler('CXRP:Handcuff', function()
    player = source
    local user_id = vRP.getUserId(player)
...
RegisterServerEvent('CXRP:SeizeWeapons2')
AddEventHandler('CXRP:SeizeWeapons2', function()
  local source = source
    player = source
    local user_id = vRP.getUserId(player)
...
RegisterServerEvent('CXRP:JailPlayer')
AddEventHandler('CXRP:JailPlayer', function()
    player = source
    local user_id = vRP.getUserId(player)
    if user_id ~= nil and vRP.hasPermission(user_id, "police.armoury") then
...
RegisterServerEvent('CXRP:UnJailPlayer')
AddEventHandler('CXRP:UnJailPlayer', function()
    player = source
    local user_id = vRP.getUserId(player)
...
RegisterServerEvent('CXRP:sendFine')
AddEventHandler('CXRP:sendFine', function()
    player = source
    local user_id = vRP.getUserId(player)
...
RegisterNetEvent("CXRP:PutPlrInVeh")
AddEventHandler("CXRP:PutPlrInVeh", function()
    player = source
    local user_id = vRP.getUserId(player)
    if user_id ~= nil and vRP.hasPermission(user_id, "police.armoury") then
    vRPclient.getNearestPlayer(player,{10},function(nplayer)
...
RegisterNetEvent("CXRP:TakeOutOfVehicle")
AddEventHandler("CXRP:TakeOutOfVehicle", function()
    player = source
    local user_id = vRP.getUserId(player)
    if user_id ~= nil and vRP.hasPermission(user_id, "police.armoury") then
    vRPclient.getNearestPlayer(player,{10},function(nplayer)
...
RegisterNetEvent("CXRP:SearchPlayer")
AddEventHandler("CXRP:SearchPlayer", function()
    player = source
    if user_id ~= nil and vRP.hasPermission(user_id, "police.armoury") then
