local lang = vRP.lang
local cfg = module("CXVehicles", "cfg/cfg_inventory")
    ...
    --   TriggerClientEvent('ECP:ToggleNUIFocus', player, false)
      vRP.prompt(player,lang.inventory.trash.prompt({vRP.getInventoryItemAmount(user_id,idname)}),"",function(player,amount)
        ...
        --   TriggerClientEvent('ECP:ToggleNUIFocus', player, true)
          TriggerEvent('CXRP:RefreshInventory', vRP.getUserSource(user_id))
          ...
          vRP.prompt(player,lang.inventory.give.prompt({vRP.getInventoryItemAmount(user_id,idname)}),"",function(player,amount)
            ...
            if new_weight <= vRP.getInventoryMaxWeight(nuser_id) then
              if vRP.tryGetInventoryItem(user_id,idname,amount,true) then
                ...
                TriggerEvent('CXRP:RefreshInventory', player)
                TriggerEvent('CXRP:RefreshInventory', nplayer)
...
function vRP.giveInventoryItem(user_id,idname,amount,notify)
  ...
  if data and amount > 0 then
    ...
    TriggerClientEvent('CXRP:FetchInventoryCL', vRP.getUserSource(user_id))
