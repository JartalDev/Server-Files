local cfg_inventory = module("CXVehicles", "cfg/cfg_inventory")
local lang = vRP.lang

RegisterServerEvent("CXRP:AskID")
AddEventHandler("CXRP:AskID",function()
    local player = source
...
RegisterServerEvent("CXRP:GiveMoney2")
AddEventHandler("CXRP:GiveMoney2",function()
    local player = source
...
RegisterNetEvent('CXRP:TrunkOpened')
AddEventHandler('CXRP:TrunkOpened', function()
...
RegisterNetEvent('CXRP:SearchPlr')
AddEventHandler("CXRP:SearchPlr", function()
  player = source
  vRPclient.getNearestPlayer(player,{5},function(nplayer)
    local nuser_id = vRP.getUserId(nplayer)
    if nuser_id ~= nil then
      -- TriggerClientEvent('CXRP:handsUpNearest', nplayer)
...
RegisterServerEvent('CXRP:searchNearestPlayer')
AddEventHandler('CXRP:searchNearestPlayer', function(nplayer)
  vRPclient.notify(nplayer,{lang.police.menu.check.checked()})
  vRPclient.getWeapons(nplayer,{},function(weapons)
    -- prepare display data (money, items, weapons)
    local money = vRP.getMoney(nuser_id)
...
RegisterServerEvent("CXRP:SendPayment")
AddEventHandler('CXRP:SendPayment', function(playerid, price)
    local source = source
    ...
    if recieverid == nil then
        ...
        if userid == recieverid then 
            ...
            if vRP.tryBankPayment(userid, tonumber(price)) then 
                ...
                TriggerClientEvent("CXRP:PlaySound", reciever, 1)
