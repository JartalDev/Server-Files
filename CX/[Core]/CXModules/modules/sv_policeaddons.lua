...
RegisterNetEvent('CXRP:policeDrag')
AddEventHandler('CXRP:policeDrag', function()
    ...
    if user_id ~= nil and vRP.hasPermission({user_id, "police.menu"}) then
        ...
        vRPclient.getNearestPlayer(source,{2},function(nplayer)
            ...
            if nuser_id ~= nil then
                ...
                if handcuffed then
                    TriggerClientEvent("CXRP:drag2", nplayer, source)
                else
...
local playerHandcuffed = false
 ...
 RegisterNetEvent("CXRP:policeCuff")
 AddEventHandler("CXRP:policeCuff", function()
     local player = source
     local user_id = vRP.getUserId({source})
     handcuffed = nil
...
RegisterNetEvent('CXRP:policeCuffStaff')
AddEventHandler('CXRP:policeCuffStaff', function(staffmode)
  local player = source
  local user_id = vRP.getUserId({source})
  handcuffed = nil
...
RegisterServerEvent('CXRP:toggleTrafficMenu')
AddEventHandler('CXRP:toggleTrafficMenu', function()
    local source = source
    local user_id = vRP.getUserId({source})
    if user_id ~= nil and vRP.hasPermission({user_id, "police.menu"}) then
        print("vRP Identification Accepted") 
        TriggerClientEvent("CXRP:openTrafficMenu", source)
    end
end)

RegisterServerEvent('CXRP:zoneActivated')
AddEventHandler('CXRP:zoneActivated', function(message, speed, radius, x, y, z)
    TriggerClientEvent('chatMessage', -1, message)
    TriggerClientEvent('CXRP:createZone', -1, speed, radius, x, y, z)
end)

RegisterServerEvent('CXRP:disableZone')
AddEventHandler('CXRP:disableZone', function(blip)
    TriggerClientEvent('CXRP:removeBlip', -1)
end)
...
RegisterServerEvent('CXRP:Fine')
AddEventHandler('CXRP:Fine', function() 
    player = source
...
RegisterServerEvent('CXRP:TrafficMenuLogs')
AddEventHandler('CXRP:TrafficMenuLogs', function(objspawned) 
  local player = source
