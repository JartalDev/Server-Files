...
function CXRPcfgdrugsServer.IronGather()
...
function CXRPcfgdrugsServer.IronCanProcess()
  local user_id = vRP.getUserId({source})
  return vRP.hasPermission({user_id, "iron.job"}),
  vRP.getInventoryItemAmount({user_id, 'ironore'}) >= 4
end

function CXRPcfgdrugsServer.IronDoneProcessing()
  local user_id = vRP.getUserId({source})
...
function CXRPcfgdrugsServer.SellIronJob(amount)
 local user_id = vRP.getUserId({source})
