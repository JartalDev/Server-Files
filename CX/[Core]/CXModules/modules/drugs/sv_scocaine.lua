...
function CXRPcfgdrugsServer.CocaineGather()
...
function CXRPcfgdrugsServer.CocaineCanProcess()
  local user_id = vRP.getUserId({source})
  return vRP.hasPermission({user_id, "cocaine.job"}),
  vRP.getInventoryItemAmount({user_id, 'cocoa'}) >= 4
end

function CXRPcfgdrugsServer.CocaineDoneProcessing()
  local user_id = vRP.getUserId({source})
...
function CXRPcfgdrugsServer.SellCocaineJob(amount)
 local user_id = vRP.getUserId({source})
