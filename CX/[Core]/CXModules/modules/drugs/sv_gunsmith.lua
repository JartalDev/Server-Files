...
function CXRPcfgdrugsServer.DiamondGather()
...
function CXRPcfgdrugsServer.DiamondCanProcess()
  local user_id = vRP.getUserId({source})
  return vRP.hasPermission({user_id, "diamond.job"}),
  vRP.getInventoryItemAmount({user_id, 'crystal'}) >= 4
end

function CXRPcfgdrugsServer.DiamondDoneProcessing()
  local user_id = vRP.getUserId({source})
...
function CXRPcfgdrugsServer.SellDiamondJob(amount)
 local user_id = vRP.getUserId({source})
