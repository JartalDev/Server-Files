...
RegisterNetEvent('CXRP:getHouses')
AddEventHandler('CXRP:getHouses', function()
    local houses = {}
    local source = source
    ...
    exports['ghmattimysql']:execute("SELECT * FROM `vrp_user_homes` WHERE user_id = @user_id", {user_id = user_id}, function(result)
        if result ~= nil then 
            ...
            TriggerClientEvent('CXRP:HousingTable', source, houses)
        end
    end)
end)
...
AddEventHandler("JudHousing:OpenChest", function(house)
    ...
    getUserByAddress(house, 1, function(huser_id)
        if huser_id == user_id then --check if homeowner is user
            TriggerClientEvent("CXRP:OpenHomeStorage", player, true , house) --JamesUK inventory modified by me
        --print(house)
        else
