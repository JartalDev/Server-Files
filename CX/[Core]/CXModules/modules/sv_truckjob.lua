...
RegisterNetEvent('CXRP:startTruckJob')
AddEventHandler('CXRP:startTruckJob', function()
    local source = source
...
RegisterNetEvent('CXRP:finishTruckJob')
AddEventHandler('CXRP:finishTruckJob', function(jobname)
    local source = source
    local user_id = vRP.getUserId({source})
    if user_id ~= nil then
