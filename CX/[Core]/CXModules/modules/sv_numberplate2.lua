...
RegisterNetEvent('CXRP:getCars')
AddEventHandler('CXRP:getCars', function()
    local cars = {}
    local source = source
    ...
    exports['ghmattimysql']:execute("SELECT * FROM `vrp_user_vehicles` WHERE user_id = @user_id", {user_id = user_id}, function(result)
        if result ~= nil then 
            ...
            TriggerClientEvent('CXRP:carsTable', source, cars)
        end
    end)
end)

RegisterNetEvent("CXRP:ChangeNumberPlate")
AddEventHandler("CXRP:ChangeNumberPlate", function(vehicle)
    local user_id = vRP.getUserId({source})
	...
	vRP.prompt({source,"Plate Name:","",function(source, plateName)
		...
		if vRP.tryFullPayment({user_id,50000}) then
			...
			TriggerClientEvent("CXRP:RecieveNumberPlate", source, plateName)
			TriggerClientEvent("CXRP:PlaySound", source, 1)
			TriggerEvent('CXRP:getCars')
		else
