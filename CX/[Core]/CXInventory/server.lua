...
AddEventHandler('CXRP:FetchPersonalInventory', function()
    local source = source
    if not InventorySpamTrack[source] then
        ...
        if data and data.inventory then
            ...
            TriggerClientEvent('CXRP:FetchPersonalInventory', source, FormattedInventoryData, vRP.computeItemsWeight({data.inventory}), vRP.getInventoryMaxWeight({UserId}))
...
AddEventHandler('CXRP:RefreshInventory', function(source)
    local UserId = vRP.getUserId({source}) 
    local data = vRP.getUserDataTable({UserId})
    if data and data.inventory then
        local FormattedInventoryData = {}
...
RegisterNetEvent('CXRP:GiveItem')
AddEventHandler('CXRP:GiveItem', function(itemId, itemLoc)
    local source = source
...
RegisterNetEvent('CXRP:TrashItem')
AddEventHandler('CXRP:TrashItem', function(itemId, itemLoc)
    local source = source
    if not itemId then  vRPclient.notify(source, {'~r~You need to select an item, first!'}) return end
    if itemLoc == "Plr" then
...
RegisterNetEvent('CXRP:FetchTrunkInventory')
AddEventHandler('CXRP:FetchTrunkInventory', function(spawnCode, vehid)
    local source = source
    ...
    vRP.getSData({carformat, function(cdata)
        ...
        TriggerClientEvent('CXRP:SendSecondaryInventoryData', source, FormattedInventoryData, vRP.computeItemsWeight({cdata}), maxVehKg)
    end})
end)

RegisterNetEvent('CXRP:FetchHouseInventory')
AddEventHandler('CXRP:FetchHouseInventory', function(nameHouse)
    local source = source
    houseName = nameHouse
...
RegisterNetEvent('CXRP:LockPick')
AddEventHandler('CXRP:LockPick', function()
    local user_id = vRP.getUserId({source})
    if vRP.tryGetInventoryItem({user_id, "lockpick", 1, true}) then
        TriggerClientEvent('CXRP:whatIsThis', source)
    end  
end)

RegisterNetEvent('CXRP:UseItem')
AddEventHandler('CXRP:UseItem', function(itemId, itemLoc)
    local source = source
    if not itemId then    vRPclient.notify(source, {'~r~You need to select an item, first!'}) return end
    if itemLoc == "Plr" then
...
RegisterNetEvent('CXRP:MoveItem')
AddEventHandler('CXRP:MoveItem', function(inventoryType, itemId, inventoryInfo, Lootbag)
    local source = source
    local UserId = vRP.getUserId({source}) 
    local data = vRP.getUserDataTable({UserId})
                ...
                vRP.getSData({carformat, function(cdata)
                    ...
                    if cdata[itemId] and cdata[itemId].amount >= 1 then
                        ...
                        if weightCalculation <= vRP.getInventoryMaxWeight({UserId}) then
                            ...
                            TriggerClientEvent('CXRP:SendSecondaryInventoryData', source, FormattedInventoryData, vRP.computeItemsWeight({cdata}), maxVehKg)
                            TriggerEvent('CXRP:RefreshInventory', source)
            ...
            if LootBagEntities[inventoryInfo] ~= nil then  
                if LootBagEntities[inventoryInfo].Items[itemId] then 
                    ...
                    if weightCalculation <= vRP.getInventoryMaxWeight({UserId}) then
                        ...
                        local maxVehKg = 200
                        TriggerClientEvent('CXRP:SendSecondaryInventoryData', source, FormattedInventoryData, vRP.computeItemsWeight({LootBagEntities[inventoryInfo].Items}), maxVehKg)                
                        TriggerEvent('CXRP:RefreshInventory', source)
                    else 
                        vRPclient.notify(source, {'~r~You do not have enough inventory space.'})
                    end
...
RegisterNetEvent('CXRP:MoveItemX')
AddEventHandler('CXRP:MoveItemX', function(inventoryType, itemId, inventoryInfo, Lootbag)
    local source = source
    local UserId = vRP.getUserId({source}) 
    local data = vRP.getUserDataTable({UserId})
    if InventoryCoolDown[source] then vRPclient.notify(source, {'~r~Inventory Cooldown.'}) return end
    ...
    if data and data.inventory then
        ...
        if inventoryType == "CarBoot" then
            --InventoryCoolDown[source] = true;
            TriggerClientEvent('CXRP:ToggleNUIFocus', source, false)
            vRP.prompt({source, 'How many ' .. vRP.getItemName({itemId}) .. 's. Do you want to move?', "", function(player, Quantity)
                Quantity = parseInt(Quantity)
                TriggerClientEvent('CXRP:ToggleNUIFocus', source, true)
                if Quantity >= 1 then
                    local carformat = "chest:u1veh_" .. inventoryInfo .. '|' .. UserId
                    vRP.getSData({carformat, function(cdata)
                        cdata = json.decode(cdata) or {}
...
RegisterNetEvent('CXRP:MoveItemAll')
AddEventHandler('CXRP:MoveItemAll', function(inventoryType, itemId, inventoryInfo, vehid)
    local source = source
    local UserId = vRP.getUserId({source}) 
    local data = vRP.getUserDataTable({UserId})
    if not itemId then  vRPclient.notify(source, {'~r~You need to select an item, first!'}) return end
...
AddEventHandler('vRP:InComa', function()
    ...
    vRPclient.isInComa(source, {}, function(in_coma) 
        if in_coma then
            ...
            TriggerEvent('CXRP:StoreWeaponsRequest', source)
...
RegisterNetEvent('CXRP:CloseLootbag')
AddEventHandler('CXRP:CloseLootbag', function()
    local source = source
    for i,v in pairs(LootBagEntities) do 
...
function CloseInv(source)
    TriggerClientEvent('CXRP:InventoryOpen', source, false, false)
end
...
function OpenInv(source, netid, LootBagItems)
    ...
    TriggerClientEvent('CXRP:InventoryOpen', source, true, true)
    local FormattedInventoryData = {}
    ...
    if vRP.hasPermission({UserId, "pd.armory"}) then
        ...
        local maxVehKg = 200
        TriggerClientEvent('CXRP:SendSecondaryInventoryData', source, FormattedInventoryData, vRP.computeItemsWeight({LootBagItems}), maxVehKg)
