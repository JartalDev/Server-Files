...
if (settingsjson.settings.StatusEnabled) {
    setInterval(() => {
        ...
        exports.sql.execute("SELECT * FROM `vrp_user_moneys`", (result) => {
            playersSinceRelease = result.length
        });
        exports.vrp.vrpbot('getUsersByPermission', ['police.pc'], function(result) {
            if (!result.length)
                onlinePD = 0
            else
                onlinePD = result.length
        })
        exports.vrp.vrpbot('getUsersByPermission', ['staff.mode'], function(result) {
            if (!result.length)
                onlineStaff = 0
            else
                onlineStaff = result.length
        })
        exports.vrp.vrpbot('getUsersByPermission', ['nhs.menu'], function(result) {
            if (!result.length)
                onlineNHS = 0
            else
                onlineNHS = result.length
        })
        exports.sql.execute("SELECT * FROM vrp_users WHERE banned = 1", (result) => {
        ...
        channelid.fetchMessage(settingsjsons.messageid).then(msg => {
            ...
            channelid.send('Status Page For FGS Is Starting..').then(id => {
    ...
    if (cmd) {
        if (message.channel.name.includes('bot') || message.channel.name.includes('auction') || cmd.conf.name == 'houseauction' || cmd.conf.name == 'embed' || cmd.conf.name == 'rulechange' || cmd.conf.name == 'auction') {
            ...
            try {
                ...
                let embed = {
                    "title": "Error Occured!",
                    "description": "\nAn error occured. Contact Paradise in vRP with the code:\n\n```" + err.message + "\n```",
                    "color": 13632027
                }
