...
exports.runcmd = async(fivemexports, client, message, params) => {
    ...
    fivemexports.sql.execute("SELECT * FROM `vrp_user_ids` WHERE user_id = ?", [params[0]], (discordid) => {
        ...
        fivemexports.sql.execute("SELECT * FROM vrp_user_moneys WHERE user_id = ?", [params[0]], (result) => {
            if (result) {
                bank = result[0].bank
                wallet = result[0].wallet
                fivemexports.sql.execute("SELECT * FROM `vrp_users` WHERE id = ?", [params[0]], (userdata) => {
                    if (userdata) {
                        ...
                        fivemexports.sql.execute("SELECT * FROM `vrp_user_data` WHERE user_id = ?", [params[0]], (result) => {
                            if (result.length > 0) {
                        ...
                        fivemexports.sql.execute("SELECT * FROM `vrp_users` WHERE id = ?", [params[0]], (result) => {
                            if (result) {
