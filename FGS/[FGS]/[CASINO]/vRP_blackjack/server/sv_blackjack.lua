...
RegisterNetEvent("recieve_chips:casino", function()
    local source = source
    local user_id = vRP.getUserId({source})
    local rows = exports['sql']:executeSync("SELECT chips FROM casino WHERE user_id = @user_id", {user_id = user_id})
    if rows[1].chips == nil then
        TriggerClientEvent("Blackjack:recChips",source, 0)
    else
        TriggerClientEvent("Blackjack:recChips",source, rows[1].chips)
    end
end)


function highRollerCheck(user_id)
    ...
    if highrollers == 1 then
        return true
    elseif highrollers == 0 then
        return false
    else
        return false
    end
    ...
    if gameId ~= nil and betAmount ~= nil and chairId ~= nil then 
        ...
        if not blackjackGameInProgress[gameId] then
            if tonumber(betAmount) then
                ...
                if betAmount > 1000000000 then
                    TriggerClientEvent("blackjack:notify",source,"~r~Error, bet amount too high.")
                    return
                end
                if betAmount > 0 then
                    if betAmount > 100000 then
                        if highRollerCheck(user_id) then
                            ...
                            TriggerClientEvent("blackjack:notify",source,"~p~Dealer:, Go buy highrollers to gamble big boy money.")
                        end
                    else
                        if tryTakeChips(source,betAmount) then
                            if blackjackGameData[gameId][source] == nil then
                                                ...
                                                if currentHand > 21 then
                                                    ...
                                                    if lostAmount > 10000000 then
                                                        TriggerClientEvent('chat:addMessage', -1, {
                                                            template = '<div style="padding: 0.5vw; margin: 0.4vw; background-color: rgba(255, 0, 0, 0.9); border-radius: 4px;"><i class="fas fa-globe"></i> Diamond Casino | ' .. GetPlayerName(source) .. ' Has LOST £' .. tostring(getMoneyStringFormatted(lostAmount)) .. '</div>',
                                                            args = { playerName, msg }
                                                          })
                                                    end
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            if potentialPushAmount > 10000000 then
                                                                TriggerClientEvent('chat:addMessage', -1, {
                                                                    template = '<div style="padding: 0.5vw; margin: 0.4vw; background-color: rgba(255, 0, 0, 0.9); border-radius: 4px;"><i class="fas fa-globe"></i> Diamond Casino | ' .. GetPlayerName(source) .. ' Has WON £' .. tostring(getMoneyStringFormatted(potentialPushAmount)) .. '</div>',
                                                                    args = { playerName, msg }
                                                                  })                  
                                                            end
                                                        end
                                                ...
                                                elseif currentHand == dealerHand then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            if potentialPushAmount > 10000000 then
                                                                TriggerClientEvent('chat:addMessage', -1, {
                                                                    template = '<div style="padding: 0.5vw; margin: 0.4vw; background-color: rgba(255, 0, 0, 0.9); border-radius: 4px;"><i class="fas fa-globe"></i> Diamond Casino | ' .. GetPlayerName(source) .. ' Has LOST £' .. tostring(getMoneyStringFormatted(potentialPushAmount)) .. '</div>',
                                                                    args = { playerName, msg }
                                                                  })                                                           
                                                            end
                                                        end
