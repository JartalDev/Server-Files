...
local Inventory = module("FGS-CARS", "cfg/inventory")
local Housing = module("FGS-Core/cfg/cfg_houses")
...
RegisterNetEvent('FGS:FetchPersonalInventory')
AddEventHandler('FGS:FetchPersonalInventory', function()
    local source = source
    if not InventorySpamTrack[source] then
        ...
        if data and data.inventory then
            ...
            TriggerClientEvent('FGS:FetchPersonalInventory', source, FormattedInventoryData, vRP.computeItemsWeight({data.inventory}), vRP.getInventoryMaxWeight({UserId}))
...
AddEventHandler('FGS:RefreshInventory', function(source)
    local UserId = vRP.getUserId({source}) 
    local data = vRP.getUserDataTable({UserId})
    if data and data.inventory then
        local FormattedInventoryData = {}
...
RegisterNetEvent('FGS:GiveItem')
AddEventHandler('FGS:GiveItem', function(itemId, itemLoc)
    local source = source
...
RegisterNetEvent('FGS:TrashItem')
AddEventHandler('FGS:TrashItem', function(itemId, itemLoc)
    local source = source
    if not itemId then  vRPclient.notify(source, {'~r~You need to select an item, first!'}) return end
    if itemLoc == "Plr" then
...
RegisterServerEvent("FGS:flashLights")
AddEventHandler("FGS:flashLights", function(nearestVeh)
    local nearestVeh = nearestVeh
    TriggerClientEvent("FGS:flashCarLightsAlarm", -1, nearestVeh)

end) 

RegisterNetEvent('FGS:FetchTrunkInventory')
AddEventHandler('FGS:FetchTrunkInventory', function(spawnCode, vehid)
    local source = source
    ...
    vRP.getSData({carformat, function(cdata)
        ...
        TriggerClientEvent('FGS:SendSecondaryInventoryData', source, FormattedInventoryData, vRP.computeItemsWeight({cdata}), maxVehKg)
    end})
end)

RegisterNetEvent('FGS:FetchHouseInventory')
AddEventHandler('FGS:FetchHouseInventory', function(nameHouse)
    local source = source
    houseName = nameHouse
...
RegisterNetEvent('FGS:LockPick')
AddEventHandler('FGS:LockPick', function()
    local user_id = vRP.getUserId({source})
    if vRP.tryGetInventoryItem({user_id, "lockpick", 1, true}) then
        TriggerClientEvent('FGS:whatIsThis', source)
    end  
end)

RegisterNetEvent('FGS:UseItem')
AddEventHandler('FGS:UseItem', function(itemId, itemLoc)
    local source = source
    if not itemId then    vRPclient.notify(source, {'~r~You need to select an item, first!'}) return end
    if itemLoc == "Plr" then
...
RegisterNetEvent('FGS:MoveItem')
AddEventHandler('FGS:MoveItem', function(inventoryType, itemId, inventoryInfo, Lootbag)
    local source = source
    local UserId = vRP.getUserId({source}) 
    local data = vRP.getUserDataTable({UserId})
            ...
            if LootBagEntities[inventoryInfo] ~= nil then  
                if LootBagEntities[inventoryInfo].Items[itemId] then 
                    ...
                    if weightCalculation <= vRP.getInventoryMaxWeight({UserId}) then
                        ...
                        local maxVehKg = 200
                        TriggerClientEvent('FGS:SendSecondaryInventoryData', source, FormattedInventoryData, vRP.computeItemsWeight({LootBagEntities[inventoryInfo].Items}), maxVehKg)                
                        TriggerEvent('FGS:RefreshInventory', source)
                    else 
                        vRPclient.notify(source, {'~r~You do not have enough inventory space.'})
                    end
...
RegisterNetEvent('FGS:MoveItemX')
AddEventHandler('FGS:MoveItemX', function(inventoryType, itemId, inventoryInfo, Lootbag)
    local source = source
    local UserId = vRP.getUserId({source}) 
    local data = vRP.getUserDataTable({UserId})
    if InventoryCoolDown[source] then vRPclient.notify(source, {'~r~Inventory Cooldown.'}) return end
...
RegisterNetEvent('FGS:MoveItemAll')
AddEventHandler('FGS:MoveItemAll', function(inventoryType, itemId, inventoryInfo, vehid)
    local source = source
    local UserId = vRP.getUserId({source}) 
    local data = vRP.getUserDataTable({UserId})
    if not itemId then  vRPclient.notify(source, {'~r~You need to select an item, first!'}) return end
...
AddEventHandler('vRP:InComa', function()
    ...
    vRPclient.isInComa(source, {}, function(in_coma) 
        if in_coma then
            ...
            TriggerEvent('ERP:StoreWeaponsRequest', source)
...
RegisterNetEvent('FGS:CloseLootbag')
AddEventHandler('FGS:CloseLootbag', function()
    local source = source
    for i,v in pairs(LootBagEntities) do 
...
function CloseInv(source)
    TriggerClientEvent('FGS:InventoryOpen', source, false, false)
end
...
function OpenInv(source, netid, LootBagItems)
    ...
    TriggerClientEvent('FGS:InventoryOpen', source, true, true)
    local FormattedInventoryData = {}
    ...
    if vRP.hasPermission({UserId, "police.armoury"}) then
        ...
        local maxVehKg = 200
        TriggerClientEvent('FGS:SendSecondaryInventoryData', source, FormattedInventoryData, vRP.computeItemsWeight({LootBagItems}), maxVehKg)
