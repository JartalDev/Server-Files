...
RegisterNetEvent('FGS:policeHotkeysTrue')
AddEventHandler('FGS:policeHotkeysTrue', function() 
    handcuffed = true
end)

RegisterNetEvent('FGS:policeHotkeysFalse')
AddEventHandler('FGS:policeHotkeysFalse', function() 
    handcuffed = false
end)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if IsControlPressed(0, 19, true) and IsControlPressed(0, 90, true) then
...
RegisterNetEvent("FGS:cuffAnimPolice")
AddEventHandler("FGS:cuffAnimPolice", function(nplayer, player)
    local player = player
    local nplayer = nplayer
    FGS_server_callback("FGS:PlayWithinDistance", 0.2 , "cuffing", 1, GetEntityCoords(GetPlayerPed(-1))) 
...
RegisterNetEvent("FGS:cuffAnimPerp")
AddEventHandler("FGS:cuffAnimPerp", function()
...
RegisterNetEvent("FGS:drag")
AddEventHandler('FGS:drag', function(pl)
    other = pl
    drag = not drag
end)

RegisterNetEvent("FGS:undrag")
AddEventHandler('FGS:undrag', function(pl)
    drag = false
end)
