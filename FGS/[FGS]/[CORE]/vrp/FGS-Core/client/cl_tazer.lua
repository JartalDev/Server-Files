...
function stunGun()
    ...
    AddTextComponentString("~y~Axon 2: ~w~You Currently Have Prongs Attached To You...")
...
RegisterCommand("cid", function()
    ...
    if GetSelectedPedWeapon(ped) == GetHashKey("weapon_stungun") then
        --FGS_server_callback("FGS:Tazering")
        SetNotificationTextEntry("STRING")
        ...
        --FGS_server_callback("FGS:Tazering2")
        SetNotificationTextEntry("STRING")
    ...
    while true do
        ...
        if IsPedShooting(ped) then
            if GetSelectedPedWeapon(ped) == GetHashKey("weapon_stungun") then
                PDSIDEPED = ped
                Wait(500)
                --TriggerEvent("FGS:PDSideTazer")
                SetNotificationTextEntry("STRING")
                AddTextComponentString("\n~y~Alert: ~w~Your Cartridge 1 Need Reloading...")
                SetNotificationBackgroundColor(140)
                SetNotificationMessage("CHAR_WE", "axoncid", false, 1, "CID", "")
                DrawNotification(false, true)
                Wait(500)
...
RegisterNetEvent("FGS:PDSideTazer")
AddEventHandler("FGS:PDSideTazer", function()
    SetNotificationTextEntry("STRING")
...
RegisterNetEvent("FGS:CivSideTazer")
AddEventHandler("FGS:CivSideTazer", function()
    SetNotificationTextEntry("STRING")
