...
RMenu.Add('CryptoMiner', 'main', RageUI.CreateMenu("Crypto Systems", "~g~Crypto Systems",1250,100))
    ...
    if RageUI.Visible(RMenu:Get('CryptoMiner', 'main')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            RageUI.Button("Manage Systems", "", {}, true, function(Hovered, Active, Selected) 
                if Selected then 
                    TriggerServerEvent("FGS:CRYPTO:ReceiveMiners")
                end
    ...
    if RageUI.Visible(RMenu:Get('CryptoMiner', 'buy_systems_manage')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
            ...
            RageUI.Button("Buy", '£'..GetMoneyString2(selected_system.price), {}, true, function(Hovered, Active, Selected)
                if Selected then 
                    FGS_server_callback('FGS:buy_crypto_system', selected_system)
                end
    ...
    if RageUI.Visible(RMenu:Get('CryptoMiner', 'manage_systems_sub')) then
        RageUI.DrawContent({ header = false, glare = false, instructionalButton = false}, function()
            ...
            RageUI.Button("Withdraw BTC", '£'..roundToWholeNumber((selected_managed_system.amountmined) * cryptoprice), {}, true, function(Hovered, Active, Selected)
                if Selected then 
                    FGS_server_callback('FGS:Withdraw:Crypto', selected_managed_system)
                end
            end, RMenu:Get("CryptoMiner", "main"))
            RageUI.Button("Sell Machine","Amount Refunded: £".. roundToWholeNumber(cfg.systems[selected_managed_system.pc_id].price / 4), {}, true, function(Hovered, Active, Selected)
                if Selected then
                    FGS_server_callback('FGS:Sell:System', selected_managed_system)
                end
            end, RMenu:Get("CryptoMiner", "main"))
...
RegisterNetEvent('FGS-Core:setCryptoPrice', function(price)
    cryptoprice = price
end)

RegisterNetEvent('FGS:CRYPTO:SetMiners', function(miners)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(50000)
        TriggerServerEvent('FGS-Core:getCryptoPrice')
    end
end)
...
Citizen.CreateThread(function() 
    while true do
        ...
        if isInArea(cfgcoords, 1.4) then 
            ...
            if IsControlJustPressed(0, 51) then 
                FGS_server_callback('FGS:CRYPTO:EXIT')
            end
        end
        Citizen.Wait(0)
    end
...
Citizen.CreateThread(function() 
    while true do
        ...
        if isInArea(cfgcoords, 1.4) then 
            ...
            if IsControlJustPressed(0, 51) then 
                FGS_server_callback('FGS:CRYPTO:ENTER')
            end
        end
        Citizen.Wait(0)
    end
...
Citizen.CreateThread(function()
    local x,y,z = table.unpack(cfg.coords) -- cords here
    local intid = GetInteriorAtCoords(x,y,z)
    ActivateInteriorEntitySet(intid, "serverroom_1server")
    ActivateInteriorEntitySet(intid, "serverroom_2server")
    ActivateInteriorEntitySet(intid, "serverroom_3server")
    ActivateInteriorEntitySet(intid, "serverroom_4server")
    ActivateInteriorEntitySet(intid, "serverroom_5server")
    ActivateInteriorEntitySet(intid, "serverroom_6server")
    ActivateInteriorEntitySet(intid, "serverroom_7server")
    ActivateInteriorEntitySet(intid, "serverroom_8server")
end)