		...
		if not takeHostage.InProgress and canTakeHostage then			
			...
			if closestPlayer then
				...
				if targetSrc ~= -1 then
					...
					FGS_server_callback("TakeHostage:sync", targetSrc)
	...
	while true do 
		...
		if takeHostage.type == "agressor" then
			...
			if IsEntityDead(ped) then	
				...
				FGS_server_callback("TakeHostage:releaseHostage", takeHostage.targetSrc)
			end 
			...
			elseif IsDisabledControlJustPressed(0,74) then --kill 			
				...
				FGS_server_callback("TakeHostage:killHostage", takeHostage.targetSrc)
				FGS_server_callback("TakeHostage:stop",takeHostage.targetSrc)
				Wait(100)
