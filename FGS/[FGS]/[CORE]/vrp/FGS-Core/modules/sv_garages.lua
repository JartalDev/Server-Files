...
local vehicle_groups2 = cfg.locksandimports
...
MySQL.createCommand("vRP/check_plate","SELECT * FROM vrp_user_vehicles WHERE user_id = @user_id AND vehicle = @vehicle")
...
RegisterNetEvent('vRP:FetchCars2')
AddEventHandler('vRP:FetchCars2', function(owned, type)
    local source = source
    local user_id = vRP.getUserId(source)
    local returned_table = {}
    if user_id then
        if not owned then
            for i, v in pairs(vehicle_groups2) do
                local noperms = false;
                local config = vehicle_groups2[i]._config
                if config.vtype == type then 
                    local perm = config.permissions or nil
                    if perm then
                        for i, v in pairs(perm) do
        ...
        for a, z in pairs(v) do
            if a ~= "_config" and a == vehicle then
                if vRP.tryFullPayment(user_id,z[2]) then 
                    ...
                    MySQL.execute("vRP/add_vehicle", {user_id = user_id, vehicle = vehicle, registration = "P VIP CAR"})
                    return 
                else 
...
--RegisterNetEvent("vRP:GetVehicleOut", function(veh)
--    local user_id = vRP.getUserId(source)
--    local money = vRP.getMoney(user_id) + vRP.getBankMoney(user_id)
--    local tax = 0
--
--    if money > 100000000 then 
--        tax = math.floor(money * 0.0008)
--    elseif money > 10000000 then 
--        tax = math.floor(money * 0.0005)
--    elseif money > 0 then 
--        tax = math.floor(money * 0.0003)
--    end
--
--    if vRP.tryFullPayment(user_id, tax) then 
--        vRPclient.notify(source,{"~g~ Paid £" .. tax .. " for Vehicle Tax."})
--    end
--end)
                                                ...
                                                if #pvehicle > 0 then
                                                    ...
                                                    MySQL.query("vRP/check_rented", {user_id = playerID, vehicle = veh}, function(pvehicles)
                                                        if #pvehicles > 0 then 
                                                            ...
                                                            MySQL.query("vRP/check_plate", {user_id = playerID, vehicle = veh}, function(pvehicles)
                                                                if pvehicles[1] then 
                                                                    if true then 
                                                                        ...
                                                                        vRPclient.notify(player,{"~r~This vehicle could not be rented."}) 
                                                                    end
                                                                else
                                                                    vRPclient.notify(player,{"~r~This vehicle could not be rented.\nError Happened"}) 
                                                                end
            ...
            for i, v in pairs(vehicle_groups) do
                ...
                for a, z in pairs(v) do
                    if a ~= "_config" and veh.vehicle == a then
                        ...
                        for i2, v2 in pairs(vehicle_groups2) do 
                            local config = vehicle_groups2[i2]._config
                            for a, z in pairs(v2) do
                                if a ~= "_config" and veh.vehicle == a then
                                    if not returned_table[i2] then 
                                        returned_table[i2] = {
                                            ["config"] = config
                                        }
                                    end
                                    if not returned_table[i2].vehicles then 
                                        returned_table[i2].vehicles = {}
                                    end
                                    local time = tonumber(veh.rentedtime) - os.time()
                                    local datetime = ""
                                    local date = os.date("!*t", time)
                                    ...
                                    returned_table[i2].vehicles[a] = {z[1], datetime}
                                end
                            end
                        end
                    end
                        ...
                        for i2, v2 in pairs(vehicle_groups2) do 
                            ...
                            for a, z in pairs(v2) do
                                if a ~= "_config" and veh.vehicle == a then
                                    ...
                                    returned_table[i2].vehicles[a .. ':' .. veh.user_id] = {z[1], datetime, veh.user_id, a}
                                end
                            end
                        end
                    end
