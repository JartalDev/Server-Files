                ...
                if amount > 0 then
                    ...
                    if user_id ~= nil then
                        if vRP.tryWithdraw(user_id, amount) then
                            vRPclient.notify(source, {"~g~£"..GetMoneyString(amount).." ~w~withdrawn."})
                        else
                            vRPclient.notify(source, {"~r~You do not have enough money in your bank."})
                        end
                    end
                else
                    vRPclient.notify(source, {"~r~Invalid value."})
                end
            end
        end
    else
                ...
                if amount > 0 then
                    ...
                    if user_id ~= nil then
                        if vRP.tryDeposit(user_id, amount) then
                            vRPclient.notify(source, {"~g~£"..GetMoneyString(amount).." ~w~deposited."})
                        else
                            vRPclient.notify(source, {"~r~You do not have enough money in your bank."})
                        end
                    end
