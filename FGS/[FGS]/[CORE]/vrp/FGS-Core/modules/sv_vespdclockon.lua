RegisterServerEvent("PoliceMenu2:ClockOn")
AddEventHandler('PoliceMenu2:ClockOn', function(policerank)
    local user_id = vRP.getUserId(source)
    local coords = GetEntityCoords(GetPlayerPed(source))
    local comparison = vector3(-1099.0737304688,-840.53088378906,19.001468658447)
...
RegisterServerEvent("PoliceMenu2:CheckPermissions")
AddEventHandler('PoliceMenu2:CheckPermissions', function()
    local user_id = vRP.getUserId(source)
    local coords = GetEntityCoords(GetPlayerPed(source))
    local comparison = vector3(-1099.0737304688,-840.53088378906,19.001468658447)
    if #(coords - comparison) > 20 then
    ...
    if vRP.hasPermission(user_id, "clockon.menu") then
        TriggerClientEvent('PoliceDuty2:Allowed', source, true)
    else
        TriggerClientEvent('PoliceDuty2:Allowed', source, false)
    end
end)

RegisterServerEvent("PoliceMenu2:ClockOff")
AddEventHandler('PoliceMenu2:ClockOff', function()
    local user_id = vRP.getUserId(source)
    local coords = GetEntityCoords(GetPlayerPed(source))
    local comparison = vector3(-1099.0737304688,-840.53088378906,19.001468658447)
