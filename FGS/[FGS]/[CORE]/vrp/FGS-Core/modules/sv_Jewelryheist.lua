...
AddEventHandler("jewelryrobberry:serverstart", function()
    ...
    if policeCount >= 2 then
        ...
        vRPclient.notify(source, {"~r~Not enough cops online."})
    end
end)


function tvRP.JRobbery2()
    local source = source
    local user_id = vRP.getUserId(source)
    local player = vRP.getUserSource(user_id)
    ...
    if not IsRobbing then 
        return print("[vrp_jewelryheist] - IsRobbing is false |" .. user_id .. " could be cheating or something.")
    end
    if Counts > 30 then 
        return print("[vrp_jewelryheist] - Counts is greater than 30 |" .. user_id .. " could be cheating or something.")
    end
    Counts = Counts + 1
    if(user_id)then
...
RegisterNetEvent("FGS:Sellj", function()
    local user_id = vRP.getUserId(source)
    local amount = vRP.getInventoryItemAmount(user_id, "Jewelry")
    local amounttogive = math.floor(amount * 4500)
    if vRP.tryGetInventoryItem(user_id, "Jewelry", amount, true) then 
        vRP.giveMoney(user_id, amounttogive)
    end
end)
