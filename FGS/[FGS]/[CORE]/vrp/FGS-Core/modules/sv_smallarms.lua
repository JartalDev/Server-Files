...
RegisterServerEvent("SmallMenu:buywap")
AddEventHandler('SmallMenu:buywap', function(price, hash, wepname)
    local source = source
    for k, v in pairs(smallcfg.gunshops) do 
        userid = vRP.getUserId(source)
        if smallcfg.perm ~= nil then 
            if vRP.hasPermission(userid, smallcfg.perm) then
                if vRP.tryPayment(userid, price) then
                    ...
                    TriggerClientEvent("SmallMenu:menu", source, false)
...
RegisterServerEvent("small:BuyItem")
AddEventHandler("small:BuyItem", function(name, price)
    local source = source
    local coords = GetEntityCoords(GetPlayerPed(source))
...
RegisterServerEvent("SmallMenu:buyammo")
AddEventHandler('SmallMenu:buyammo', function(price, hash, wepname)
    local source = source
    for k, v in pairs(smallcfg.gunshops) do 
        local x,y,z = table.unpack(v.large)
        local largecoords = vector3(x,y,z)
        ...
        if smallcfg.perm ~= nil then 
            if vRP.hasPermission(userid, smallcfg.perm) then
                if vRP.tryPayment(userid, price) then
                    TriggerClientEvent("SmallMenu:GiveAmmo", source, hash)
                else 
                    TriggerClientEvent("SmallMenu:menu", source, false)
                    vRPclient.notify(source, {"~r~Insufficient funds"})
                end
...
RegisterServerEvent("FGS:GetWhitelistsmall")
AddEventHandler('FGS:GetWhitelistsmall', function()
    local source = source
    ...
    MySQL.query("vRP/get_whitelists", {user_id = userid}, function(whitelists, affected)
        if #whitelists > 0 then 
            ...
            for k,v in pairs(whitelists) do 
                if v.location == "Small" then 
                    table.insert(temp, v)
                end
            end
            TriggerClientEvent('FGS:ShowWhitelistsSmall', source, temp)
        end
    end)
end)