...
function tvRP.varyHealth(variation)
    ...
    if variation == 200 then 
        TriggerEvent("FGS:FixPlayer",ped)
        return
    end
...
Citizen.CreateThread(function() -- coma thread
    ...
    while true do
        ...
        if IsEntityDead(GetPlayerPed(-1)) and not in_coma then --Wait for death check
            ...
            TriggerEvent("FGS:Storeweapons")   
            TriggerEvent("FGS:10SECS")
            TriggerEvent("FGS:IsPlayerDead")
            tvRP.ejectVehicle()
        ...
        if in_coma then
            if not calledNHS then
				if IsControlJustPressed(1, 51) then
					...
					FGS_server_callback('FGS:recieveNHSCall2')
				end
			end
            scaleform = Initialize("mp_big_message_freemode")
		    DrawScaleformMovieFullscreen(scaleform, 255, 255, 255, 255, 0)
            if not calledNHS then
				DrawAdvancedText(0.605, 0.581, 0.005, 0.0028, 0.37, "Press [E] to call the NHS", 255, 255, 255, 255, 6, 0)
                DrawAdvancedTextOutline(0.605, 0.554, 0.005, 0.0028, 0.37, "You are dying... Call the NHS now", 255, 255, 255, 255, 6, 0)
            elseif calledNHS then 
...
RegisterNetEvent("FGS:10SECS")
AddEventHandler("FGS:10SECS", function()
    DeathAnim = 5
    ...
    while true do
        ...
        if IsEntityDead(PlayerPedId()) then
            ...
            if DeathReason == 'committed suicide' or DeathReason == 'died' then
                --FGS_server_callback('DiscordBot:PlayerDied', GetPlayerName(PlayerId()) .. ' ' .. DeathReason .. '.', Weapon)
                DeathString = "~r~You committed suicide"
            else
                --FGS_server_callback('DiscordBot:PlayerDied', GetPlayerName(Killer) .. ' ' .. DeathReason .. ' ' .. GetPlayerName(PlayerId()) .. '.', Weapon)
...
function IsPistol(Weapon)
    local Weapons = {'WEAPON_SNSPISTOL', 'WEAPON_HEAVYPISTOL', 'WEAPON_VINTAGEPISTOL', 'WEAPON_PISTOL', 'WEAPON_APPISTOL', 'WEAPON_COMBATPISTOL', 'WEAPON_M1911', 'WEAPON_TX22', 'WEAPON_SR40', 'WEAPON_REVOFRG', 'WEAPON_FNX', 'WEAPON_NAILGUNFRG'}
    for i, CurrentWeapon in ipairs(Weapons) do
        if GetHashKey(CurrentWeapon) == Weapon then
            return true
        end
...
function IsRifle(Weapon)
    local Weapons = {'WEAPON_AK200', 'WEAPON_MOSIN', 'WEAPON_SCARL', 'WEAPON_CARBINERIFLE', 'WEAPON_MUSKET', 'WEAPON_ADVANCEDRIFLE', 'WEAPON_ASSAULTRIFLE', 'WEAPON_ANARCHYLR300FRG', 'WEAPON_MK4', 'WEAPON_M13RAYZ'}
    for i, CurrentWeapon in ipairs(Weapons) do
        if GetHashKey(CurrentWeapon) == Weapon then
            return true
        end
...
WeaponNames = {
    ...
    [tostring(GetHashKey("WEAPON_TX22"))] = 'Taurus TX22',
    [tostring(GetHashKey("WEAPON_SR40"))] = 'Ruger SR40',
    [tostring(GetHashKey("WEAPON_FNX"))] = 'FNX .45',
    [tostring(GetHashKey("WEAPON_NAILGUNFRG"))] = 'Nail Gun',
    [tostring(GetHashKey("WEAPON_REVOFRG"))] = 'Revolver',
    [tostring(GetHashKey("WEAPON_GLOCK17"))] = 'Glock',

    --Assault Rifles
    [tostring(GetHashKey("WEAPON_AK200"))] = 'AK-200',
    [tostring(GetHashKey("WEAPON_ANARCHYLR300FRG"))] = 'Anarchy LR300',
    [tostring(GetHashKey("WEAPON_MK4"))] = 'MK4 Special Carbine',
    [tostring(GetHashKey("WEAPON_M13RAYZ"))] = 'M13',
    [tostring(GetHashKey("WEAPON_SCARL"))] = 'SCAR-L',
    [tostring(GetHashKey("WEAPON_MCX"))] = 'Sig-MCX',
    [tostring(GetHashKey("WEAPON_G36K"))] = 'G36K',
    [tostring(GetHashKey("WEAPON_M4A1"))] = 'M4A1',
    [tostring(GetHashKey("WEAPON_AK74"))] = 'AK-74',
    [tostring(GetHashKey("WEAPON_RUST"))] = 'Rust AK-74',
    [tostring(GetHashKey("WEAPON_AK74KASHNAR"))] = 'AK Fire Serpent',
    ...
    [tostring(GetHashKey("WEAPON_SVD"))] = 'SVD Sniper',
...
Citizen.CreateThread(function()
  while true do 
    Wait(40000)
    --FGS_server_callback("FGS:SAVEUSERDATA")
  end
end)
