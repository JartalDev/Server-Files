        ...
        function(player, choice)
            ...
            if user_id ~= nil then
                if vRP.tryGetInventoryItem(user_id, fullidname, 1, true) then -- give weapon body
                    local weapons = {}
                    local amount = vRP.getInventoryItemAmount(user_id, "wammo|"..args[2])
                    if amount > 250 then
                        amount = 250
                    end
                    vRP.tryGetInventoryItem(user_id, "wammo|"..args[2], amount, true)

                    weapons[args[2]] = {ammo = amount}
                        ...
                        vRPclient.getWeapons(
                            ...
                            function(uweapons)
                                --if uweapons[args[2]] ~= nil then -- check if the weapon is equiped
                                    vRPclient.GetCurrentWeapon(player, {}, function(curweapon)
                                        if vRPAmmoTypes[vRPWeaponHash[tonumber(curweapon)]] then 
                                            for k,v in pairs(uweapons) do 
                                                if (vRPAmmoTypes[vRPWeaponHash[tonumber(curweapon)]] == vRPAmmoTypes[k:lower()]) and (vRPAmmoTypes[args[2]:lower()] == vRPAmmoTypes[k:lower()]) then 
                                                    if vRP.tryGetInventoryItem(user_id, fullidname, ramount, true) then -- give weapon ammo
                                                        local weapons = {}
                                                        weapons[vRPWeaponHash[tonumber(curweapon)]] = {ammo = ramount}
                                            ...
                                            vRPclient.notify(player,{"~r~You are not holding any weapon."})
                                        end
                                    end)
                                --else
                                --    vRPclient.notify(player,{"~r~You do not have a weapon for this ammo."})
                                --end
                            end
                        )
                    end
