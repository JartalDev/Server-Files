...
function MySQL.execute(queryname, variables)
    if Queries[queryname] then 
        if variables then 
            exports['sql']:execute(Queries[queryname], variables)
        else 
            exports['sql']:execute(Queries[queryname])
        end 
    else 
...
function MySQL.asyncQuery(queryname, variables)
    if Queries[queryname] then 
        if variables then 
            local rows = exports['sql']:executeSync(Queries[queryname], variables)
            return rows
        end 
    else 
        --print('VRP_DB_DRIVER: Error query does not exist!')
...
function MySQL.query(queryname, variables, cb)
    if Queries[queryname] then 
        if variables then 
            exports['sql']:execute(Queries[queryname], variables, function(rows, affected)
                if cb then 
                    cb(rows, affected)
                end 
            end)
