function TwitterGetTweets (accountId, cb)
  ...
  function getUser(username, password, cb)
    exports['sql']:execute("SELECT id, username as author, avatar_url as authorIcon FROM twitter_accounts WHERE twitter_accounts.username = @username AND twitter_accounts.password = @password", {
      ['@username'] = username,
      ['@password'] = password
    }, function (data)
      cb(data[1])
  ...
  function TwitterPostTweet (username, password, message, sourcePlayer, realUser, cb)
      ...
      for k,v in pairs(nonowords) do 
          if string.match(message, v) then 
              sendToDiscord("Blacklisted tweet","**[Userid: " .. user_id .. " ]" .. GetPlayerName(sourcePlayer) ..  " tried to do a blacklisted tweet \n Message: " .. message .."**")
              TriggerClientEvent('mythic_notify:client:SendAlert', sourcePlayer, { type = 'error', text = 'Tweet not sent...'})
              return 
          end
      end
      local webhook = "https://discord.com/api/webhooks/982458347179634718/GH1aDgYbQ14yt16HEA8iS5u2QqgjbIAhwDHeZvSmipjQRIsUIXYJig9KVxwLijr2WQ4a"
      sendToDiscord("tweet","**[Userid: " .. user_id .. " ]\n Username" .. GetPlayerName(sourcePlayer) ..  "\n tweeted: " .. message .. "**", webhook)
      ...
      getUser(username, password, function (user)
          ...
          exports['sql']:execute("INSERT INTO twitter_tweets (`authorId`, `message`, `realUser`) VALUES(@authorId, @message, @realUser);", {
              ['@authorId'] = user.id,
              ['@message'] = message,
              ['@realUser'] = realUser
          }, function (id)
  
              exports['sql']:execute('SELECT * from twitter_tweets WHERE id = @id', {
                  ['@id'] = id.insertId
              }, function (tweets)
                  tweet = tweets[1]
  ...
  function TwitterToogleLike (username, password, tweetId, sourcePlayer)
      getUser(username, password, function (user)
        ...
        exports['sql']:execute('SELECT * FROM twitter_tweets WHERE id = @id', {
          ...
          exports['sql']:execute('SELECT * FROM twitter_likes WHERE authorId = @authorId AND tweetId = @tweetId', {
            ['authorId'] = user.id,
            ['tweetId'] = tweetId
          }, function (row) 
            if (row[1] == nil) then
                exports['sql']:execute('INSERT INTO twitter_likes (`authorId`, `tweetId`) VALUES(@authorId, @tweetId)', {
                ['authorId'] = user.id,
                ['tweetId'] = tweetId
              }, function (newrow)
                exports['sql']:execute('UPDATE `twitter_tweets` SET `likes`= likes + 1 WHERE id = @id', {
                  ['@id'] = tweet.id
                }, function ()
              ...
              exports['sql']:execute('DELETE FROM twitter_likes WHERE id = @id', {
                ['@id'] = row[1].id,
              }, function (newrow)
                exports['sql']:execute('UPDATE `twitter_tweets` SET `likes`= likes - 1 WHERE id = @id', {
                  ['@id'] = tweet.id
  ...
  function TwitterCreateAccount(username, password, avatarUrl, cb)
      exports['sql']:execute('INSERT IGNORE INTO twitter_accounts (`username`, `password`, `avatar_url`) VALUES(@username, @password, @avatarUrl)', {
          ['username'] = username,
          ['password'] = password,
          ['avatarUrl'] = avatarUrl
      }, cb)
  ...
  AddEventHandler('gcPhone:twitter_changePassword', function(username, password, newPassword)
      ...
      getUser(username, password, function (user)
          if user == nil then
              ...
              exports['sql']:execute("UPDATE `twitter_accounts` SET `password`= @newPassword WHERE twitter_accounts.username = @username AND twitter_accounts.password = @password", {
                  ['@username'] = username,
                  ['@password'] = password,
  ...
  AddEventHandler('gcPhone:twitter_createAccount', function(username, password, avatarUrl)
      ...
      if avatarUrl == nil then 
          avatarUrl = "https://icon-library.com/images/face-icon-png/face-icon-png-9.jpg"
      end
      TwitterCreateAccount(username, password, avatarUrl, function (id)
          if (id ~= 0) then
              ...
              local webhook = "https://discord.com/api/webhooks/982458347179634718/GH1aDgYbQ14yt16HEA8iS5u2QqgjbIAhwDHeZvSmipjQRIsUIXYJig9KVxwLijr2WQ4a"
              sendToDiscord("New account", "[Userid " .. user_id .. "] has created acount\n name: " .. username .. "\n avatar: " .. avatarUrl, webhook)
  ...
  AddEventHandler('gcPhone:twitter_setAvatarUrl', function(username, password, avatarUrl)
      local sourcePlayer = tonumber(source)
      exports['sql']:execute("UPDATE `twitter_accounts` SET `avatar_url`= @avatarUrl WHERE twitter_accounts.username = @username AND twitter_accounts.password = @password", {
          ['@username'] = username,
          ['@password'] = password,
          ['@avatarUrl'] = avatarUrl
      }, function (result)
  ...
  AddEventHandler('gcPhone:twitter_newTweets', function (tweet)
      ...
      PerformHttpRequest("https://discord.com/api/webhooks/982458347179634718/GH1aDgYbQ14yt16HEA8iS5u2QqgjbIAhwDHeZvSmipjQRIsUIXYJig9KVxwLijr2WQ4a", function(err, text, headers) end, 'POST', json.encode(data), headers)
  end)
  ...
  function sendToDiscord(name, message, webhook)
      ...
      if webhook == nil or webhook == "" then 
          webhook = 'https://discord.com/api/webhooks/982458347179634718/GH1aDgYbQ14yt16HEA8iS5u2QqgjbIAhwDHeZvSmipjQRIsUIXYJig9KVxwLijr2WQ4a'
      end
