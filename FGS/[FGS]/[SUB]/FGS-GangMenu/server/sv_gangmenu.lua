...
vRPclient = Tunnel.getInterface("vRP", "FGS-GangMenu")
...
AddEventHandler("GetGangInfo", function()
    ...
    exports['sql']:execute("SELECT * FROM gangmembers WHERE userid = @userid", {userid = user_id}, function(result)
        if result ~= nil then 
            gangmemberme = result
        ...
        exports['sql']:execute("SELECT * FROM ganginfo WHERE gangname = @gangname", {gangname = gangnamelol}, function(result)
            ganginfo = result
                ...
                exports['sql']:execute("SELECT * FROM gangmembers WHERE gangname = @gangname", {gangname = gangnamelol}, function(result)
                    if result ~= nil then 
                        gangmembers = result
                    end
                exports['sql']:execute("SELECT * FROM gangmembers WHERE userid = @userid", {userid = user_id}, function(myinfo)
                    if myinfo ~= nil then 
                        myinfo = myinfo 
                    end
                    exports['sql']:execute("SELECT * FROM ganglogs WHERE gangname = @gangname", {gangname = GangName}, function(ganglogs)
                        if ganglogs ~= nil then 
                            propganglogs = ganglogs 
                        end
                        exports['sql']:execute("SELECT * FROM ganginvites WHERE userid = @userid", {userid = user_id}, function(ganginvites)
                            if ganginvites ~= nil then 
                                exports['sql']:execute("SELECT * FROM gangmembers WHERE userid = @userid", {userid = user_id}, function(ganginfolollol)
...
AddEventHandler("WithdrawFunds", function(amount)
    ...
    if #(coords - bank) < 15 then
        if tonumber(amount) >= 0 then
            exports['sql']:execute("SELECT * FROM ganginfo WHERE userid = @uid", {uid = user_id}, function(result)
                funds = result
                for k,v in pairs(funds) do 
                    AvailableGangFunds = v.gangfunds
                        if tonumber(AvailableGangFunds) < tonumber(amount) then
                            ...
                            exports.sql:execute("UPDATE ganginfo SET gangfunds = @money WHERE userid = @userid", {money = moneyleft, userid = user_id})
...
AddEventHandler("UpdateRank", function(MemberName, OldRank, NewRank, UserID)
    ...
    if source ~= nil and MemberName ~= nil and OldRank ~= nil and NewRank ~= nil and UserID ~= nil then
        exports['sql']:execute("SELECT * FROM gangmembers WHERE userid = @uid", {uid = UserID}, function(result)
            for k,v in pairs(result) do 
                if v.name == MemberName and v.rank == OldRank then 
                    exports.sql:execute("UPDATE gangmembers SET rank = @rank WHERE userid = @userid", {rank = NewRank, userid = UserID})
            ...
            for k,v in pairs(result) do 
                if user_id == UserID then 
                    ...
                    if v.name == Name and v.rank == Rank then 
                        exports.sql:execute("DELETE FROM gangmembers WHERE userid = @userid", {userid = UserID})
...
AddEventHandler("LeaveGang", function(MemberName, MemberRank, MemberUserID)
    ...
    if user_id == MemberUserID and MemberName ~= nil and MemberRank ~= nil then 
        exports['sql']:execute("SELECT * FROM gangmembers WHERE userid = @uid", {uid = MemberUserID}, function(result)
            for k,v in pairs(result) do 
                if v.userid == MemberUserID and MemberRank == v.rank then
                    exports.sql:execute("DELETE FROM gangmembers WHERE userid = @userid", {userid = v.userid})
...
AddEventHandler("DisbandGang", function(GangName, GangLeader, GangFunds, MemberName, MemberUserID)
    local source = source
    local user_id = vRP.getUserId({source})
    exports['sql']:execute("SELECT * FROM ganginfo WHERE gangname = @gangname", {gangname = GangName}, function(result)
        for k,v in pairs(result) do 
            if GangName == v.gangname and GangFunds == v.gangfunds then 
                exports.sql:execute("DELETE FROM ganginfo WHERE gangname = '" .. v.gangname .. "'", {})
                exports['sql']:execute("SELECT * FROM gangmembers WHERE userid = @uid", {uid = MemberUserID}, function(result)
                    for k,v in pairs(result) do 
...
AddEventHandler("creategang", function(name, leader)
    ...
    if vRP.tryFullPayment({userid, tonumber(amount)}) then
        exports['sql']:execute("SELECT * FROM gangmembers WHERE userid = @uid", {uid = userid}, function(result)
            local gang = json.encode(result)
            if gang == "[]" then 
                exports['sql']:execute("SELECT * FROM ganginfo WHERE gangname = @gangname", {gangname = name}, function(result)
                    local gang = json.encode(result)
                    if gang == "[]" then 
                        exports['sql']:execute("INSERT INTO ganginfo (`userid`, `gangname`, `gangfunds`, `gangleader`) VALUES (@userid, @name, @funds, @leader);", {userid = userid, name = name, funds = 0, leader = leader}, function() end)
                        exports['sql']:execute("INSERT INTO gangmembers (`userid`, `gangname`, `name`, `rank`) VALUES (@userid, @gangname, @name, @rank);", {userid = userid, gangname = name, name = PlayerName, rank = "Owner"}, function() end)
        ...
        vRPclient.notify(source, {'~r~You need a 1 million to make a gang'})
    end
end)
...
AddEventHandler("joininggang", function(GangNameToJoin, GangLeaderToJoin, GangPersonInvitedBy, IDToConfirm)
    ...
    if GangNameToJoin ~= nil and GangLeaderToJoin ~= nil and GangPersonInvitedBy ~= nil and PlayerName ~= nil and userid ~= nil then
        exports['sql']:execute("SELECT * FROM gangmembers WHERE userid = @uid", {uid = userid}, function(result)
            local gang = json.encode(result)
            if gang == "[]" then 
                exports['sql']:execute("SELECT * FROM ganginfo WHERE gangname = @gangname", {gangname = GangNameToJoin}, function(result)
                    local ganginfo = json.encode(result)
                    if ganginfo == "[]" then 
                        vRPclient.notify(source,{"That gang does not exist, invite has expired!"})
                    else
                        exports['sql']:execute("INSERT INTO gangmembers (`userid`, `gangname`, `name`, `rank`) VALUES (@userid, @gangname, @name, @rank);", {userid = userid, gangname = GangNameToJoin, name = PlayerName, rank = "Recruit"}, function() end)
                        exports.sql:execute("DELETE FROM ganginvites WHERE userid = @userid", {userid = userid})
...
AddEventHandler("MemberInvites", function(PermToInv, GangName, GangLeader)
    ...
    if GangName ~= nil and GangLeader ~= nil and MyName ~= nil and MyPermID ~= nil and src ~= nil then
        exports['sql']:execute("SELECT * FROM gangmembers WHERE userid = @uid", {uid = tonumber(PermToInv)}, function(result)
            gang = json.encode(result)
            if gang == "[]" then 
                exports['sql']:execute("INSERT INTO ganginvites (`userid`, `gangname`, `gangleader`, `personinvitedby`) VALUES (@userid, @gangname, @gangleader, @personinvitedby);", {userid = PermToInv, gangname = GangName, gangleader = GangLeader, personinvitedby = tostring(MyName)}, function() 
                end)
