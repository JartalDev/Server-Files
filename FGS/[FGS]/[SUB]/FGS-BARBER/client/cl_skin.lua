...
RegisterNUICallback('updateSkin', function(data)
	...
	if v == true then
		...
		TriggerServerEvent("FGS:saveFaceData",faceSaveData)
	end
end)
...
RegisterNetEvent("FGS:setHairstyle")
AddEventHandler("FGS:setHairstyle",function(hairstyle)
    if hairstyle then
    ...
    while true do
        ...
        for k, v in pairs(barberShops) do
			if isInArea(v, 100.0) then
				DrawMarker(2, v, 0, 0, 0, 0, 0, 0, 0.3, 0.3, 0.3, 0, 255, 255, 255, 0, true, false)
			end

            if isInArea(v, 1.0) then
				alert("Press ~INPUT_PICKUP~ get a haircut!")
